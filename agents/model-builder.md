...
- `proof-parse-shape-unrecognized` / `parse-shape-unrecognized` means tool defect: report the diagnostics and stop. No `pbi_dax_query`, no `pbi_model_refresh`, no `probeData:false`, no primitive writes, no empty-model claim.
...
- A blocked, incomplete, or parse-shape-unrecognized Date proof is a stop condition. Do not use `pbi_dax_query`, `pbi_model_refresh`, manual DAX, `probeData:false`, or primitive Date/relationship writes as fallbacks.
