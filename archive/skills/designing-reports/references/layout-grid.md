...
- **Page title** (textbox): the confirmed subject or metric.
- **Visual titles**: context that differentiates the visual, using verified dimensions/grain.
- **Subtitles**: almost always redundant — hide by default when a visual title is set

Bad: repeating the same confirmed metric/dimension in the page title, visual title, and subtitle. Good: page title names the subject; visual titles add distinct verified context such as the chosen dimension or grain. Hide axis titles when the axis label is self-evident.
...
## Selecting Verified Time Granularity for Trend Visuals

When adding a trend visual, match the time axis to the validated metric contract and model grain. For semantic-model date fields, require `pbi_model_plan_date_table` / `pbi_model_plan_date_grain` evidence; prompt wording, active filters, or user confirmation are intent signals, not grain proof.

| Verified context | Trend granularity | Date column to use |
|---|---|---|
| Planner proves annual/monthly context and monthly decision cadence | Monthly | Verified month or month-year field |
| Planner proves quarterly/period context | Confirmed monthly, weekly, or period field | Verified date-table field at that grain |
| Planner proves daily monitoring grain | Daily or confirmed operational grain | Verified governed date key or daily axis field |
| No confirmed date context | Block and ask, or use the existing visual grain unchanged | No new date field guessed |

If planner proof is unavailable, block the new date-axis decision or preserve the existing visual grain unchanged. Do not default to monthly in production.
