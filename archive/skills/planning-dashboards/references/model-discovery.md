...
## Data Dictionary and Measure Intent Grounding

Use `pbi_data_dictionary_get` or the INFO views below to prove model metadata and canonical refs; use the user's data dictionary/glossary to prove business meaning. Model metadata alone does not confirm a measure intent; only explicit user or glossary evidence can mark it `confirmed`. If a needed metric, target, RAG threshold, or time-intelligence policy remains `draft`, return `needs-user-input` with `clarifyingQuestions`; do not infer a formula from table names, field names, sample values, or domain convention.
...
FILTER(
    ...
    [REFERENCED_OBJECT_TYPE] = "MEASURE"
        && [REFERENCED_TABLE] = "<MeasureTable>"
        && [REFERENCED_OBJECT] = "<MeasureName>"
)
```
