...
Full checklist for Power BI report reviews. Six dimensions, scored on a P0–P3 severity scale. Use this catalog when running `reviewing-reports` — every finding must be attributed to actual tool output. This catalog frames what the report-side review surface checks; report-side validation predicates live under `packages/core/src/pbir/validators.ts` and related PBIR validator modules, not the semantic-model BPA engine.
...
**Check:** Use `pbi_report_validate` / report BPA output for stale role/projection findings. If no tool surfaces the issue, report it as not verified rather than re-walking PBIR JSON by hand.
...
The report-side Best Practice Analyzer runs a rule sweep over the report and reports violations grouped by rule with affected visual/page paths and severity (info/warning/error). It is the structural counterpart to schema/JSON validation. Report-side rule predicates live under `packages/core/src/pbir/validators.ts` and related PBIR validator modules; this catalog frames what that surface checks. Coverage:
