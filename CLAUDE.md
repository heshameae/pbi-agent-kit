...
## Workflow
- This is a standard npm workspaces monorepo (npm, not pnpm).
- Tests: `npm test` runs both packages (core + mcp); keep all green. `npm run lint` runs Biome.
- Build marker: after editing anything under `packages/*/src` or the root `package.json`, run `npm run build` (core builds before mcp) so `dist` and the build marker match the source; otherwise the offline launcher fails closed on a fresh checkout. Keep `packages/{core,mcp}/dist` and the marker committed.
