...
This map is now grouped by **skill → `references/<file>.md`** (lean SKILL.md router + on-demand references), not the old flat 13-skill list. Each of the 7 task-grouped skills is a thin SKILL.md pointing at `references/*.md` (where the mined content lives) and `scripts/` (executable/data). Non-skill destinations (agents, hooks, MCP/engine, packaging, pipelines, thin CRUD) carry over unchanged. **Authoritative skill layout:** `docs/superpowers/plans/2026-05-20-skill-architecture.md`.
...
Source-repo short names also used in tables below: ruiromano = ruiromano/powerbi-agentic-plugins · dg1/dg2/dg3/dg4 = data-goblin set · sff = microsoft/skills-for-fabric · ac = awesome-copilot-pbi-data · pbm = powerbi-master (claude-plugin-marketplace) · borghei = borghei/Claude-Skills · antigravity = sickn33/antigravity-awesome-skills · agent-skills = practicalswan/agent-skills · awesome-llm-apps = shubhamsaboo/awesome-llm-apps · plugin-master = powerbi-master/plugin-master (claude-plugin-marketplace).
...
### Skills (7) → references/
| Skill | Reference files | Pri | Section |
|---|---|---|---|
| `authoring-measures` | dax-performance.md · dax-query-rules.md · advanced-templates.md · time-intelligence.md · calc-groups.md | P0–P1 | Model |
| `modeling-semantic-model` | tmdl-grammar.md · columns-relationships.md · naming.md · power-query-m.md · rls.md · ai-readiness.md | P0–P1 | Model |
| `designing-reports` | layout-grid.md · theme-cascade.md · kpi-cards.md · tables.md · chart-selection.md · audience-styles.md | P1 | Report |
| `authoring-svg-visuals` | svg-doctrine.md · extension-measures.md · scripts/examples/ | P0–P1 | Report |
| `reviewing-models` | check-catalog.md · output-format.md | P1 | Model |
| `reviewing-reports` | check-catalog.md · output-format.md | P1 | Report |
| `planning-dashboards` | intake-protocol.md · metric-contract.md · model-discovery.md | P1 | Infra |
...
The 7 task-grouped skills. Each is a lean SKILL.md router; rows below are the mined content to land in each `references/*.md`. Every "Path in repo" is grep-verified against `packed/*.xml`.
...
**Role:** DAX perf catalog + FE/SE engine model, valid-query/authoring rules, advanced templates, time-intelligence, calc groups. **Build:** new `skills/authoring-measures/`. **Loaded by:** model-builder; ref data-analyst, report-builder.

### references/dax-performance.md
...
### references/advanced-templates.md
| What to put here | Source repo | Path in repo | Packed XML | Action | Pri |
|---|---|---|---|---|---|
...
**Shared with:** modeling-semantic-model (tmdl-grammar emission, advanced-features role/calc emission), reviewing-models (DAX001/002/004 + valid-query → BPA predicates, TI detector for DQ-TI rule), gate-measure-create (qualify-cols / valid-query rules), planning-dashboards (target DAX). **Deep detail:** powerbi-agentic-plugins.md (DAX framework, detector, off-by-one), dg3-semantic-models.md (date-table, MDL005/006), awesome-copilot-pbi-data.md, claude-plugin-marketplace-pbi.md, dg4-te-fabric-hooks-root.md (§14 calc-group).

---

## modeling-semantic-model · skill
**Role:** TMDL grammar/emission, column+relationship rules, naming, Power Query M, RLS, AI-readiness. **Build:** new `skills/modeling-semantic-model/`. **Loaded by:** model-builder; read data-analyst.

### references/tmdl-grammar.md
| What to put here | Source repo | Path in repo | Packed XML | Action | Pri |
|---|---|---|---|---|---|
...
### references/columns-relationships.md
| What to put here | Source repo | Path in repo | Packed XML | Action | Pri |
|---|---|---|---|---|---|
...
**Shared with:** authoring-measures (advanced-features emission, TI item bodies = single canonical copy), reviewing-models (object-properties enums = valid-value sets; ai-readiness checklist = reviewer checks; naming-regex), authoring-measures/calc-groups (advanced-features + cookbook), BPA engine (RLS/PQ/naming rule intents), gate-measure-create (measure-metadata rules), planning-dashboards (date-table correctness via dg3 ai-readiness.md). **Deep detail:** dg1-pbip.md (§ TMDL + object-properties + annotations), skills-for-fabric-authoring.md, claude-plugin-marketplace-pbi.md, powerbi-agentic-plugins.md, dg3-semantic-models.md (§ folding, §10 ai-readiness), awesome-copilot-pbi-data.md (§ RLS), dg4-te-fabric-hooks-root.md (§15 RLS library).

---

## designing-reports · skill
**Role:** Page layout/grid, theme cascade, KPI cards, tables, chart selection, audience styles. **Build:** new `skills/designing-reports/`. **Loaded by:** report-builder; ref data-analyst.
...
**Shared with:** authoring-svg-visuals + reviewing-reports (sentiment-token CF coordination, inline cell SVG, stale-override/compliance audit, auto-size scroll check), authoring-measures (target DAX, TI), planning-dashboards (KPI levels↔personas, audience trichotomy + storytelling, maturity vocabulary), report-builder (visual.json bindings/position/CF only; per-persona defaults), pbi_visual_bind (binding checklist mirrors validations). **Deep detail:** findings/dg2-reports.md, findings/agent-skills-design-breadth.md, findings/claude-skills-data-analytics.md, findings/antigravity-bi-bundles.md.

---

## authoring-svg-visuals · skill
**Role:** SVG visuals authored via DAX measures (Node-compatible) + reportExtensions plumbing + example measures. **Build:** new `skills/authoring-svg-visuals/`. **Loaded by:** report-builder.

### references/svg-doctrine.md
| What to put here | Source repo | Path in repo | Packed XML | Action | Pri |
|---|---|---|---|---|---|
...
### references/extension-measures.md
| What to put here | Source repo | Path in repo | Packed XML | Action | Pri |
|---|---|---|---|---|---|
...
**Shared with:** designing-reports (color tokens Text dtype, inline cell SVG), reviewing-reports (svg-reviewer checklist), pbi_visual_bind (ImageUrl-measure binding). **Deep detail:** findings/dg2-reports.md, findings/dg1-pbip.md.

---

## reviewing-models · skill
**Role:** Semantic-model review check catalog + severity/output format; deterministic-first, don't-trust-the-report. **Build:** new `skills/reviewing-models/`. **Loaded by:** model-reviewer.
...
| Cross-ref: BPA semantic rule predicates live in the engine — see ## MCP TOOLS / BPA ENGINE (`packages/core/src/modeling/bpa.ts`); this catalog frames what the engine checks. | — | — | — | — | — |
...
**Shared with:** modeling-semantic-model (ai-readiness shared dg3 file), authoring-measures (anti-patterns), BPA engine + pbi_model_check (its check catalog = the engine's rules), reviewing-reports (shared severity/output format), model-builder (frontmatter house-shape, Must/Prefer/Avoid). **Deep detail:** dg3-semantic-models.md (§9 auditor), dg1-pbip.md (§ pbip-validator), claude-plugin-marketplace-pbi.md (taxonomy), 02-architecture §4.

---

## reviewing-reports · skill
**Role:** Report design/binding/a11y/perf review check catalog + severity/P0–P3 output; two-stage review. **Build:** new `skills/reviewing-reports/`. **Loaded by:** report-reviewer.
...
| Cross-ref: report-side BPA rule predicates live in the engine — see ## MCP TOOLS / BPA ENGINE (`packages/core/src/modeling/bpa.ts`); this catalog frames what the report-BPA surface checks. | — | — | — | — | — |
...
**Shared with:** report-side BPA (bucket B feeds findings), designing-reports/theme-cascade (compliance audit; their rules = its checks), authoring-svg-visuals (svg-reviewer checklist), reviewing-models (shared severity/output format). **Deep detail:** findings/dg2-reports.md, findings/agent-skills-design-breadth.md, findings/powerbi-agentic-plugins.md.

---

## planning-dashboards · skill
**Role:** Read-only planning — clarifying intake, KPI/metric contract, INFO.* model discovery + anti-fabrication grounding. **Build:** new `skills/planning-dashboards/`. **Loaded by:** data-analyst.
...
**Shared with:** designing-reports/kpi-cards (KPI levels↔personas; card *styling* rows stay there, *definition contract* rows stay here), authoring-measures (target DAX), house-style frontmatter (§authoring); INFO.* technique also feeds model-builder/reviewer; grounding invariant shared w/ gate-measure-create + MCP immutable-context. **Deep detail:** review/01 §data-analyst; claude-skills-data-analytics.md, dg2-reports.md, skills-for-fabric-authoring.md, awesome-llm-apps-rag-eval.md.

---

# AGENTS

The 5 agents. `Loads skills:` updated to the new task-grouped skill names (per the plan's wiring table). Tool/role rows carried over from the prior map.

---

## data-analyst · agent
**Role:** read-only planner — clarifying intake, KPI contract, INFO.* model discovery, emits DashboardSpec. **Build:** new `agents/data-analyst.md`. **Loads skills:** planning-dashboards, modeling-semantic-model (read), designing-reports (ref).
...
## model-builder · agent
**Role:** implementer (model: sonnet) — creates DAX measures + exports TMDL; tool-first, self-validate, report. **Build:** new `agents/model-builder.md` (closest existing: `pbi-data-architect.md`). **Loads skills:** authoring-measures, modeling-semantic-model.
...
**Role:** reviewer (model: sonnet, tools Read/Grep/Glob[+Bash]) — grain/relationships/BPA; deterministic-first, don't-trust-the-report, severity-bucketed output. **Build:** new `agents/model-reviewer.md` (closest existing: `pbi-model-doctor.md`). **Loads skills:** reviewing-models.
...
**Shared with:** model-builder (frontmatter house-shape, Must/Prefer/Avoid), BPA engine + pbi_model_check (its check catalog = the engine's rules), modeling-semantic-model/ai-readiness (shared dg3 file). **Deep detail:** dg3-semantic-models.md (§9 auditor), dg1-pbip.md (§ pbip-validator), claude-plugin-marketplace-pbi.md (taxonomy), 02-architecture §4.

---

## report-builder · agent
**Role:** Builds report pages/visuals — ONE page per invocation; lean orchestrator. **Build:** new `agents/report-builder.md` (relates to existing `agents/pbi-designer.md`). **Loads skills:** designing-reports, authoring-svg-visuals.
...
**Shared with:** designing-reports + authoring-svg-visuals (consumes them), pbi_visual_bind (writes bindings via the MCP tool), data-analyst (receives the propose-before-building plan). **Deep detail:** findings/dg2-reports.md, findings/dg1-pbip.md, findings/agent-skills-design-breadth.md.

---

## report-reviewer · agent
**Role:** Design/binding/a11y/perf review — READ-ONLY; two-stage (spec-compliance first, then quality). **Build:** new `agents/report-reviewer.md` (relates to existing `agents/pbi-report-reviewer.md`). **Loads skills:** reviewing-reports.
...
**Shared with:** report-side BPA (bucket B feeds findings), designing-reports/theme-cascade (compliance audit), all report skills (their rules = its checks), model-reviewer (shared severity/output format). **Deep detail:** findings/dg2-reports.md, findings/agent-skills-design-breadth.md, findings/powerbi-agentic-plugins.md.
...
**Shared with:** pbi_dax_reference_check (shares the DAX-reference validation module + grounding invariant), modeling-semantic-model (measure-metadata rules). **Deep detail:** dg4-te-fabric-hooks-root.md (pbi-desktop hooks), dg1-pbip.md (validation hooks), 02-architecture §5.
...
# MCP TOOLS / BPA ENGINE

BPA rule predicates (semantic + report) live in `packages/core/src/modeling/bpa.ts` — engine data, not a skill. reviewing-models/check-catalog.md and reviewing-reports/check-catalog.md carry a one-line cross-ref back here.
...
**Shared with:** modeling-semantic-model (RLS/PQ rule intents), authoring-measures (TI detector, DAX001/002/004), pbi_model_check (runs this engine). **Deep detail:** dg4-te-fabric-hooks-root.md (§1-6), powerbi-agentic-plugins.md (BPA json), claude-plugin-marketplace-pbi.md (§ BPA catalog), dg3-semantic-models.md (§9 auditor).
...
**Shared with:** report-reviewer (bucket B = its findings), designing-reports/theme-cascade (no-hardcoded-hex rule), pbi_visual_bind (lineage/orphan-binding checks). **Deep detail:** findings/powerbi-agentic-plugins.md, findings/dg1-pbip.md, findings/dg2-reports.md.
...
**Shared with:** gate-measure-create (same DAX-reference validation logic + grounding invariant — single shared module), authoring-measures (qualification rules). **Deep detail:** powerbi-agentic-plugins.md (query guide), dg4-te-fabric-hooks-root.md (pbi-desktop validate-dax), 02-architecture §5.
...
**Shared with:** authoring-svg-visuals (ImageUrl binding), report-builder (calls this tool), reviewing-reports (binding checklist mirrors its validations). **Deep detail:** findings/dg1-pbip.md, findings/dg2-reports.md.
...
**Role:** house-style for `description:` — the single biggest routing lever; `<capability+scope>. Use when <2-4 triggers>. Does NOT… (use X).` WHEN-not-WHAT, BI-noun-dense, ≤~200 chars. **Build:** new authoring convention (apply to all skill + agent descriptions).
...
**Role:** conventions for `agents/*.md` — lean-orchestrator vs read-only-reviewer archetypes; `<example>` routing; Skill-Activation table; Self-Validation Protocol; model-per-role; greppable audit. **Build:** new authoring standard + apply to 5 agents.
