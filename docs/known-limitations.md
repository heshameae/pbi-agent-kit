...
- **Measure enumeration is version-resilient but not guaranteed complete.** The Microsoft MCP `measure_operations/List` payload shape can change between MCP versions, so measure *names* may not parse from it. The wrapper falls back to a version-independent DAX `INFO.VIEW.MEASURES()` query to enumerate measures regardless of payload shape. If that engine function is unavailable, enumeration degrades to a best-effort list flagged `measuresCaptured:false` and `pbi_model_list_measures` returns a `warning` — it never throws and never blocks writes. Measure *creation* is unaffected in all cases (the engine validates references at commit time); only *listing existing measures by name* depends on either the payload or the DAX path succeeding.
