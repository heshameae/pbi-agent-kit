...
import { GOVERNED_DATE_TABLE_ANNOTATIONS, MAX_FUTURE_HORIZON_DAYS, MeasureIntentSchema, VERSION, buildDataDictionary, buildDateGrainProbeQuery, buildDateTableCoverageProbeQuery, buildModelFieldIndexFromModel, buildTimeIntelligenceMeasureExpression, calendarOvershootsFactDay, compareByName, daxReferenceCheck, deriveRequiredDateCoverageFacts, detectCalendarMaxAnchorCap, expressionUsesTimeIntelligence, findCalendarSourceRisks, findColumn, findMeasure, hasCompleteDateGrainProof, hasCompleteDateTableKeyProof, isNumericType, isTemporalType, isYearEndDateLiteral, modelDoctor, modelDoctorFromFolder, normalizeDataType, parseBarePeriodToDate, parseDateGrainProbeResult, parseDateTableCoverageProbeResult, parseTMDLFolder, planDateGrain, planDateTableCoverage, planStarSchemaSharedDimensions, readGovernedDatePolicy, relationshipCheck, validateDashboardSpec, } from 'pbi-core';
import { z } from 'zod';
import { ModelDriver, redactConnectionSecrets, } from './model-bridge/model-driver.js';
...
function summarizeToolResult(name, result) {
    ...
    if (counts !== null && typeof counts === 'object' && !Array.isArray(counts)) {
        const rendered = Object.entries(counts)
            ...
            .join(', ');
        if (rendered)
            parts.push(`counts(${rendered})`);
    }
    return `${parts.join(' ')}. Full payload is in structuredContent.`;
}
function registerToolDefinition(target, definition) {
    target.registerTool(definition.name, {
...
function tool(name, title, description, inputShape, annotations, handler) {
    ...
    const callback = async (input) => {
        try {
            ...
            const report = err !== null && typeof err === 'object' && 'report' in err
                ? err.report
                : undefined;
            const structured = report !== undefined && report !== null && typeof report === 'object'
                ? { error: msg, ...report }
                : { error: msg };
            return {
                isError: true,
...
const MODELING_SURFACE_TOOL_NAMES = new Set([
    'pbi_model_check',
...
const LIVE_MODEL_PERSISTENCE = 'Live semantic-model metadata is updated in Power BI Desktop — press Ctrl+S in Desktop to persist the model metadata.';
...
const DATE_PROOF_PARSE_SHAPE_RATIONALE = 'A ROW()-based Date proof is constructed to emit at least one tabular row for each expected proof row even when source tables are empty; zero parsed evidence indicates an unrecognized or lost tool/host result shape, not an empty model.';
const DATE_PROOF_PARSE_SHAPE_MESSAGE = 'Date proof returned zero parsed evidence from a generated ROW()-based query. This is a tool/parse defect to report, not evidence that the model is empty; stop before Date writes and do not run alternate diagnostics as a workaround.';
function dateProofBlockedGuidance() {
    return {
...
function resolveSemanticModelDefinition(input) {
    ...
    throw new Error(`Could not locate a .SemanticModel/definition folder from "${input ?? process.cwd()}". Pass an explicit modelPath.`);
}
tool('pbi_model_check', 'Model Doctor — TMDL Validators', 'Run live-first semantic-model validators: grain inference, BPA-style DAX/modeling/formatting rules, and relationship pre-flight (missing keys, type mismatch, ambiguous paths, cycles). Uses the live Power BI Desktop/env-pinned model when available; modelPath is the offline fallback. Pass bridgeIntent { fromTable, toTable, axes? } to also compute bridge_covers / bridge_uncovered / bridge_blocked_axes for a TREATAS cross-fact analysis. Read-only; no live DAX execution.', {
    modelPath: z
        .string()
        .optional()
        .describe('Path to a .SemanticModel folder, its definition/ folder, a .pbip file, or a directory that contains a sibling .SemanticModel. Auto-detected from cwd if omitted.'),
    model: z
        .string()
        .optional()
        .describe('When MULTIPLE Power BI Desktop instances are open, the target model/file name or listed port. Omit when only one instance is open or when the connection is env-pinned.'),
    bridgeIntent: z
        .object({
        ...
        axes: z
            .array(z.string())
            .optional()
            .describe('Visual axes the user intends to slice by. Any axis not in bridge_covers ends up in bridge_uncovered.'),
    })
        .optional()
        .describe('Optional TREATAS bridge intent. When provided, the report includes a bridge analysis identifying which axes the bridge covers and which are structurally blocked.'),
    live: z
        .boolean()
        .optional()
        .describe('Force a live Power BI Desktop/env-pinned read. When true, modelPath is ignored and no offline fallback is used.'),
}, { readOnlyHint: true, idempotentHint: true }, async (input) => {
    if (input.live) {
...
tool('pbi_model_regulated_check', 'Regulated Enterprise Model Check', 'Run model doctor with regulated-enterprise evidence gates. Reports captured vs not captured security/governance metadata, blocks Copilot/data-agent exposure unless AI schema scope, RLS leakage tests, tenant settings, and approved instructions are evidenced, and treats missing policy evidence as blocked rather than clean.', {
    ...
    policyEvidence: z
        ...
        .optional()
        .describe('External audit evidence. Missing required evidence keeps regulated readiness blocked; this tool never infers bank safety from model structure alone.'),
    live: z
        .boolean()
        .optional()
        .describe('Force a live Power BI Desktop/env-pinned read. When true, modelPath is ignored and no offline fallback is used.'),
...
tool('pbi_dax_reference_check', 'Check DAX References', 'Read-only lexical DAX reference check against a .SemanticModel/definition folder. Verifies qualified Table[Field] and bare [Measure] references; fails closed on missing or ambiguous references.', {
    modelPath: z
        .string()
...
}, { readOnlyHint: true, idempotentHint: true }, (input) => {
...
let modelDriver = null;
function getModelDriver() {
    if (!modelDriver)
        modelDriver = new ModelDriver(getMsMcpClient());
    return modelDriver;
}
export function setModelDriverForTests(driver) {
    modelDriver = driver;
}
const MODEL_FOLDER_FIELD = z
    .string()
    .optional()
    .describe('Semantic-model OFFLINE fallback only. Leave UNSET whenever Power BI Desktop is open — model writes auto-target the live Desktop instance and appear immediately (the user presses Ctrl+S to persist live semantic-model metadata). Set it only for headless/offline editing of a .SemanticModel/definition when no Desktop is running; folder writes land on TMDL disk files and an already-open Desktop will not see them until the model is reopened.');
const MODEL_SELECT_FIELD = z
    .string()
    .optional()
    .describe('When MULTIPLE Power BI Desktop instances are open, the target model/file name (e.g. "Sales") or listed port (e.g. "59186"). Paths and .pbix/.SemanticModel suffixes are ignored for names, case-insensitive. Omit when only one instance is open or when the connection is env-pinned.');
const INCLUDE_COLUMNS_FIELD = z
    .boolean()
    .optional()
    .describe("Include each table's full column array. Defaults to false to keep table inventory fast and low-token; use pbi_model_list_columns for column-focused work.");
...
const RELATIONSHIP_CARDINALITY_FIELD = z
    .literal('manyToOne')
    .optional()
    .describe('Relationship cardinality. Currently only "manyToOne" is supported because fromTable is the foreign-key/many side and toTable is the primary-key/one side.');
...
function resolveConnectOpts(folderPath, model) {
    if (!folderPath && !model)
        return undefined;
    return {
...
function resolveLivePreferredConnectOpts(folderPath, model) {
...
async function connectModel(drv, folderPath, model) {
...
async function snapshotModel(folderPath, model, options = {}) {
    if (folderPath) {
        const drv = getModelDriver();
        if (await hasLiveModelCandidate(drv)) {
...
async function readDriverSnapshot(drv, modelPath, options, expectedConnection, cacheMode = 'fresh') {
    const maybeDriver = drv;
...
async function hasLiveModelCandidate(drv) {
    if (process.env.PBI_MODELING_MCP_CONNECTION_STRING?.trim())
        return true;
    if (process.env.PBI_REPORT_MCP_DISABLE_LIVE_PROBE === '1')
        return false;
    try {
...
async function snapshotForWrite(folderPath, model, options = { includeMeasures: false, includeRoles: false }) {
    const drv = getModelDriver();
    if (folderPath) {
        if (await hasLiveModelCandidate(drv)) {
            const conn = await drv.ensureConnection(resolveLivePreferredConnectOpts(folderPath, model));
...
function modelWithoutMeasures(model) {
    return {
        ...model,
...
function liveTargetSummary(mode, connection, modelSelector) {
    if (!connection) {
        return {
            mode,
...
function observedTableSummary(model) {
    return {
...
function observedTableInventorySummary(tables) {
    return {
        count: tables.length,
        sampleNames: tables
...
function hasMissingTableReason(reasons) {
    return reasons.some((reason) => reason.code !== undefined && /table.*missing|table-not-found/.test(reason.code));
}
function missingDaxReferenceTables(missing, model) {
...
function assertTableExistsForWrite(input) {
    if (input.model.tables.some((table) => table.name === input.tableName))
        return;
    throwInputValidationError(`${input.action} refused: table "${input.tableName}" was not found in the targeted model.`, {
        tool: input.tool,
...
async function tableInventoryForRead(folderPath, model, opts = {}) {
    ...
    return { mode: 'folder', tables: tableInventory(parseTMDLFolder(conn.folderPath)) };
}
function isRecord(value) {
    return value !== null && typeof value === 'object' && !Array.isArray(value);
}
function normalizeStarSchemaPlan(plan) {
    if (!isRecord(plan)) {
        return {
    ...
    const sanitizedAxisPlans = axisPlans.map((axisPlan) => {
        const axisWritePlan = Array.isArray(axisPlan.writePlan)
            ...
            : [];
        const hasExecutableCreateTable = axisWritePlan.some((item) => item.action === 'create-calculated-table');
        if (hasExecutableCreateTable || typeof axisPlan.daxExpression !== 'string')
            return axisPlan;
        const { daxExpression: _daxExpression, ...safeAxisPlan } = axisPlan;
        return safeAxisPlan;
    });
    const proposedDimensions = Array.isArray(plan.proposedDimensions) || sanitizedAxisPlans.length === 0
        ? plan.proposedDimensions
        : sanitizedAxisPlans.flatMap((axisPlan) => {
            const axisWritePlan = Array.isArray(axisPlan.writePlan)
                ? axisPlan.writePlan.filter(isRecord)
                : [];
            if (axisWritePlan.length === 0)
                return [];
            const createTable = axisWritePlan.find((item) => item.action === 'create-calculated-table');
            return [
                {
    ...
    const writePlanItems = axisPlans.flatMap((axisPlan) => Array.isArray(axisPlan.writePlan) ? axisPlan.writePlan.filter(isRecord) : []);
    const relationshipWrites = Array.isArray(plan.relationshipWrites) || writePlanItems.length === 0
    ...
    const relationshipRepairWrites = Array.isArray(plan.relationshipRepairWrites) || writePlanItems.length === 0
        ? plan.relationshipRepairWrites
        : writePlanItems
            .filter((item) => item.action === 'repair-relationships')
            .flatMap((item) => (Array.isArray(item.relationships) ? item.relationships : []));
    const hideFkWrites = Array.isArray(plan.hideFkWrites) || writePlanItems.length === 0
        ? plan.hideFkWrites
        : writePlanItems
            .filter((item) => item.action === 'hide-source-columns')
            .flatMap((item) => Array.isArray(item.columns)
            ...
            : []);
    const keyColumnWrites = Array.isArray(plan.keyColumnWrites) || writePlanItems.length === 0
...
function planStarSchemaJoin(model, opts) {
...
function routeActualsTargetsAxes(model, input) {
    ...
    const sharedAxes = dedupeStrings(left.columns.map((column) => column.name).filter((name) => rightColumnNames.has(name))).sort((a, b) => (a < b ? -1 : a > b ? 1 : 0));
    const candidates = input.axes ? dedupeStrings(input.axes) : sharedAxes;
    const dimensionAxes = [];
    const temporalAxes = [];
    const sharedTemporalAxes = [];
...
function syntheticStarSchemaPlan(leftTable, rightTable, reason) {
    return {
        design: 'star-schema-shared-dimension',
        directFactRelationshipAllowed: false,
        leftTable,
...
function discoverDateRefsForTables(input, temporalAxes) {
    return dedupeDateRefs(temporalAxes.flatMap((axis) => [
        { tableName: input.leftTable, dateColumn: axis },
        { tableName: input.rightTable, dateColumn: axis },
    ]));
}
function actualsTargetsRemainingBusinessQuestions() {
    return [
        {
            topic: 'allocation-or-missing-target-behavior',
            reason: 'Date grain and shared axes are observable from metadata/proof; allocation, carry-forward, blank/zero handling, and missing-target behavior are business policy.',
        },
    ];
}
const GOVERNED_DATE_AXIS_REQUIRED_REASON = 'Cross-fact modeling with temporal axes requires one governed Date table shared by every participating fact. Auto LocalDateTable relationships are per-column implementation details and do not provide a conformed report axis.';
function governedDateAxisRequirement(model, input) {
    const dateRefs = dedupeDateRefs(input.dateRefs);
    if (dateRefs.length === 0)
        return undefined;
    const temporalAxes = dedupeStrings(input.temporalAxes ?? []);
    const inferredEndpoint = input.effectiveDateEndpoint === undefined
        ? inferCommonGovernedDateEndpoint(model, dateRefs)
        : undefined;
    const endpoint = input.effectiveDateEndpoint ??
        (inferredEndpoint
            ? {
                ...inferredEndpoint,
...
function sharedTemporalDateAxisRequirement(model, input) {
    const axisRouting = routeActualsTargetsAxes(model, input);
    const temporalAxes = axisRouting.temporalAxes.length > 0 ? axisRouting.temporalAxes : axisRouting.sharedTemporalAxes;
    if (temporalAxes.length === 0)
        return undefined;
...
function relationshipDateEndpointForFactRef(relationship, ref) {
...
function inferCommonGovernedDateEndpoint(model, dateRefs) {
    let commonEndpoint;
    for (const ref of dateRefs) {
        const endpoints = dedupeDateRefs(model.relationships
        ...
        if (commonEndpoint.dateTable !== endpoint.tableName ||
...
function sourceBlockers(source, blockers) {
    return blockers.map((blocker) => ({ source, ...blocker }));
}
async function planActualsTargetsJoin(input) {
    const { mode, model } = await snapshotModel(input.folderPath, input.model, {
        includeMeasures: false,
        includeRoles: false,
    });
    const axisRouting = routeActualsTargetsAxes(model, input);
    const starSchemaPlan = axisRouting.dimensionAxes.length > 0
        ? planStarSchemaJoin(model, {
            leftTable: input.leftTable,
            rightTable: input.rightTable,
            axes: axisRouting.dimensionAxes,
        })
        : syntheticStarSchemaPlan(input.leftTable, input.rightTable, 'No non-temporal shared axes require shared-dimension planning.');
    const dateRefs = input.dateRefs && input.dateRefs.length > 0
        ? dedupeDateRefs(input.dateRefs)
        : discoverDateRefsForTables(input, axisRouting.temporalAxes.length > 0
            ? axisRouting.temporalAxes
            : axisRouting.sharedTemporalAxes);
    const explicitDateEndpoint = input.dateTable && input.dateColumn
        ? {
        ...
        : undefined;
    const partialDateEndpointSupplied = (input.dateTable !== undefined && input.dateColumn === undefined) ||
        (input.dateTable === undefined && input.dateColumn !== undefined);
    const inferredDateEndpoint = explicitDateEndpoint === undefined && dateRefs.length > 0
        ? inferCommonGovernedDateEndpoint(model, dateRefs)
        : undefined;
    const effectiveDateEndpoint = explicitDateEndpoint ??
    ...
    let dateGrainPlan = {
        probeStatus: {
            status: 'not-run',
            reason: 'No shared temporal columns were discovered. Provide dateRefs to run deterministic date-grain proof.',
        },
        facts: [],
        blockers: [
            {
                code: 'date-refs-required',
                message: 'No shared temporal columns were discovered between the requested tables. Pass explicit dateRefs for the actual/target date columns.',
            },
        ],
    };
    if (dateRefs.length === 0) {
        requiredInputs.push({
            topic: 'date-refs',
            reason: 'The model metadata did not expose a same-name temporal column on both tables. This is a date role selection, not a grain question.',
        });
    }
    else {
        if (partialDateEndpointSupplied) {
            requiredInputs.push({
                ...
                reason: 'Both dateTable and dateColumn are required to validate a governed shared Date axis for actuals/targets joins.',
                dateTable: input.dateTable,
                dateColumn: input.dateColumn,
            });
        }
    ...
    if (input.dateRefs === undefined && axisRouting.sharedTemporalAxes.length > 1) {
        requiredInputs.push({
            topic: 'date-role',
            reason: 'Multiple shared temporal columns were discovered. The tool proved grain for the discovered candidates, but the active actuals/targets date role is business semantics.',
    ...
    const dateTableCoverageBlockers = dateTableCoverage && !coverageReady
            ...
            : [
                {
                    ...
                    message: 'The governed Date table coverage proof did not return valid coverage for the actuals/targets date refs.',
                },
            ]
        : [];
    ...
    return {
        ...
        routedTemporalAxes: axisRouting.temporalAxes.length > 0
            ? axisRouting.temporalAxes
            : axisRouting.sharedTemporalAxes,
        dateRefs,
        ...(dateRefs.length > 0
            ? {
                dateAxisRequirement: effectiveDateEndpoint
                    ? governedDateAxisRequirement(model, {
                        dateRefs,
                        temporalAxes: axisRouting.temporalAxes.length > 0
                            ? axisRouting.temporalAxes
                            : axisRouting.sharedTemporalAxes,
...
function dateAxisRequirementFromPlan(plan) {
    return isRecord(plan.dateAxisRequirement) ? plan.dateAxisRequirement : undefined;
}
function unresolvedDateAxisRequirement(plan) {
...
function requiredStringField(record, key, context) {
...
function assertExecutableStarSchemaPlan(plan) {
    if (plan.design !== 'star-schema-shared-dimension') {
        throwInputValidationError('Star-schema apply refused: planner did not return star-schema design.', {
            gate: 'star-schema-apply',
    ...
    if (plan.directFactRelationshipAllowed !== false) {
        throwInputValidationError('Star-schema apply refused: direct fact relationships are not an executable design.', {
            gate: 'star-schema-apply',
...
function assertExplicitStarSchemaApplyAxes(axes) {
    const normalized = axes.map((axis) => axis.trim());
    if (normalized.length === 0 || normalized.some((axis) => axis.length === 0)) {
        throwInputValidationError('Star-schema apply refused: pass explicit non-empty axes for write workflows.', {
            gate: 'star-schema-apply',
...
function relationshipCreateWrites(plan) {
...
function relationshipRepairWrites(plan) {
...
function plannedStarSchemaOperations(createTableWrites, keyWrites, repairWrites, createRelationshipWrites, sourceHideWrites, refreshAfterCreate) {
    return [
...
function inferStarDimensionKeyDataTypes(model, plan) {
    ...
    for (const axisPlan of records(plan.plans)) {
        ...
        if (!axis || !leftTable || !rightTable || !dimensionTable || !dimensionKeyColumn)
            continue;
...
function commonStarDimensionKeyDataType(leftDataType, rightDataType) {
...
function withDimensionKeyDataTypes(writes, dataTypes) {
    return writes.map((write) => ({
        ...write,
        dataType: dataTypes.get(`${write.tableName}\u0000${write.name}`),
    }));
}
function normalizeDaxForComparison(expression) {
    return expression?.replace(/\s+/g, ' ').trim().toLowerCase();
}
function tableExpressionForComparison(table) {
    return normalizeDaxForComparison(table.expression ?? table.partitionSources?.find((source) => source.expression)?.expression);
}
function assertNoPartialStarSchemaDimensionArtifact(model, plan, createTableWrites) {
    for (const axisPlan of records(plan.plans)) {
        if (axisPlan.source !== 'new-calculated-table')
            continue;
        const axis = stringField(axisPlan, 'axis');
        const plannedTable = stringField(axisPlan, 'proposedDimensionTableName');
        if (!axis || !plannedTable)
            continue;
        const existingAxisTable = model.tables.find((table) => table.name !== plannedTable &&
            table.name === axis &&
            table.columns.some((column) => column.name === axis));
        if (!existingAxisTable)
            continue;
        throwInputValidationError('Star-schema apply refused: a planned generated dimension table already exists but is not covered by the planner output.', {
            gate: 'star-schema-apply',
    ...
    for (const write of createTableWrites) {
        ...
        if (!plannedExpression)
            continue;
        const existing = model.tables.find((table) => table.name !== write.name && tableExpressionForComparison(table) === plannedExpression);
        if (!existing)
            continue;
        throwInputValidationError('Star-schema apply refused: a matching generated dimension table already exists but is not covered by the planner output.', {
            gate: 'star-schema-apply',
            reason: 'partial-generated-dimension-artifact',
...
function findStarSchemaModelColumn(model, tableName, columnName) {
    return model.tables
        .find((table) => table.name === tableName)
        ?.columns.find((column) => column.name === columnName);
}
function relationshipMatches(existing, planned) {
    return (existing.fromTable === planned.fromTable &&
...
function validateAppliedStarSchemaPlan(finalModel, plan) {
    const failures = [];
    for (const write of keyColumnWrites(plan)) {
        ...
        if (column.summarizeBy !== 'none') {
            failures.push({
                code: column.summarizeBy === undefined
    ...
    for (const write of [...relationshipCreateWrites(plan), ...relationshipRepairWrites(plan)]) {
        if (!finalModel.relationships.some((relationship) => relationshipMatches(relationship, write))) {
            failures.push({
...
async function gateStarSchemaCreateRelationshipWrite(drv, connection, write) {
    const model = await readDriverSnapshot(drv, '(live)', { includeMeasures: true, includeRoles: false }, connection);
    ...
    const dateTableWarnings = await enforceDateRelationshipWriteGate('live', model, connection, write, 'create-date-relationship');
...
async function gateStarSchemaRepairRelationshipWrite(drv, connection, write) {
    const model = await readDriverSnapshot(drv, '(live)', { includeMeasures: true, includeRoles: false }, connection);
    ...
    if (existing.fromTable !== write.fromTable ||
        existing.fromColumn !== write.fromColumn ||
        existing.toTable !== write.toTable ||
        existing.toColumn !== write.toColumn) {
        throwInputValidationError('Star-schema apply refused: relationship repair target changed since planning.', {
            gate: 'star-schema-apply',
    ...
    if (existing.isActive === true && existing.crossFilteringBehavior === 'single') {
        throwInputValidationError('Star-schema apply refused: relationship repair target is no longer in a repairable state.', {
            gate: 'star-schema-apply',
    ...
    await enforceDateRelationshipWriteGate('live', model, connection, {
        id: write.id,
        fromTable: write.fromTable,
        fromColumn: write.fromColumn,
        toTable: write.toTable,
...
function modelWithStarSchemaSyntheticDimensions(model, createTableWrites, keyWrites) {
    const keyWritesByTable = new Map();
    ...
    for (const write of createTableWrites) {
        if (existingTableNames.has(write.name))
            continue;
...
function modelWithStarSchemaPreflightRelationship(model, relationship) {
    const id = 'id' in relationship
        ? relationship.id
...
function gateStarSchemaCreateRelationshipWriteOnModel(model, write) {
    const check = relationshipCheck(write, model);
    if (!check.valid) {
        throwInputValidationError('Star-schema apply refused: relationship create gate failed.', {
            gate: 'star-schema-apply',
...
function gateStarSchemaRepairRelationshipWriteOnModel(model, write) {
    const existing = model.relationships.find((relationship) => relationship.id === write.id);
    if (!existing) {
        throwInputValidationError('Star-schema apply refused: relationship repair target is missing.', {
            gate: 'star-schema-apply',
...
async function preflightStarSchemaRelationshipWrites(drv, connection, plan, createTableWrites, keyWrites, repairWrites, createRelationshipWrites) {
    const latestModel = await readDriverSnapshot(drv, '(live)', { includeMeasures: true, includeRoles: false }, connection);
    assertNoPartialStarSchemaDimensionArtifact(latestModel, plan, createTableWrites);
    let preflightModel = modelWithStarSchemaSyntheticDimensions(latestModel, createTableWrites, keyWrites);
...
async function applyStarSchemaJoin(input) {
    ...
    if (mode !== 'live') {
        throwInputValidationError('Star-schema apply refused: batched shared-dimension writes require a live Power BI Desktop model.', {
            gate: 'star-schema-apply',
            reason: 'not-live',
            mode,
        });
    ...
    const keyWrites = withDimensionKeyDataTypes(keyColumnWrites(plan), inferStarDimensionKeyDataTypes(model, plan));
    ...
    const plannedOperations = plannedStarSchemaOperations(createTableWrites, keyWrites, repairWrites, createRelationshipWrites, sourceHideWrites, input.refreshAfterCreate !== false);
    ...
    const operations = [];
    await preflightStarSchemaRelationshipWrites(drv, connection, plan, createTableWrites, keyWrites, repairWrites, createRelationshipWrites);
    ...
    for (const write of repairWrites) {
        relationshipWarnings.push(...(await gateStarSchemaRepairRelationshipWrite(drv, connection, write)));
        operations.push({
            action: 'repair-relationship',
            relationshipId: write.id,
    ...
    for (const write of createRelationshipWrites) {
        relationshipWarnings.push(...(await gateStarSchemaCreateRelationshipWrite(drv, connection, write)));
        operations.push({
            action: 'create-relationship',
            fromTable: write.fromTable,
            fromColumn: write.fromColumn,
    ...
    const finalModel = await readDriverSnapshot(drv, mode === 'live' ? '(live)' : undefined, {
        includeMeasures: true,
        includeRoles: false,
    }, connection);
    ...
    const finalDateAxisRequirement = dateAxisRequirementFromPlan(finalPlan) ?? initialDateAxisRequirement;
    const finalUnresolvedDateAxisRequirement = finalDateAxisRequirement?.status === 'governed-date-table-required'
    ...
    if (validationFailures.length > 0) {
        throwInputValidationError('Star-schema apply completed writes but final validation did not pass.', {
            gate: 'star-schema-apply',
...
function daxResultDiagnostics(raw) {
    const diagnosticRows = collectDaxDiagnosticRows(raw);
    if (raw !== null && typeof raw === 'object' && !Array.isArray(raw)) {
        const obj = raw;
        const rowCount = typeof obj.rowCount === 'number'
...
function diagnosticRowCount(diagnostics) {
    const rowCount = diagnostics.rowCount;
    return typeof rowCount === 'number' && Number.isFinite(rowCount) ? rowCount : 0;
}
function collectDaxDiagnosticRows(payload, depth = 0) {
    if (depth > 8)
        return [];
    if (Array.isArray(payload)) {
        if (payload.every((item) => isDiagnosticRowRecord(item)))
            return payload;
...
function isDiagnosticRowRecord(value) {
    if (value === null || typeof value !== 'object' || Array.isArray(value))
        return false;
    const record = value;
    return !['rows', 'data', 'value', 'values', 'results', 'tables'].some((key) => key in record);
}
function dateProofParseShapeStatus(diagnostics, parsedEvidenceRows, expectedEvidenceRows = 0) {
    if (parsedEvidenceRows > 0)
        return undefined;
    if (diagnosticRowCount(diagnostics) === 0 && expectedEvidenceRows <= 0)
        return undefined;
    return {
...
function dateProofParseShapeBlockedPayload(diagnostics, parsedEvidenceRows, expectedEvidenceRows = 0) {
    if (parsedEvidenceRows > 0)
        return undefined;
    if (diagnosticRowCount(diagnostics) === 0 && expectedEvidenceRows <= 0)
        return undefined;
...
function dateTableCoverageEvidenceRows(evidence) {
    return (evidence.dateTable ? 1 : 0) + evidence.facts.length;
}
async function planDateGrainForRead(input) {
    const { mode, model: snapshot, connection, } = await snapshotModel(input.folderPath, input.model, {
    ...
    let evidence = [];
    let coverageEvidence = { facts: [] };
    let probeStatus = {
        status: shouldProbe ? 'not-run' : 'skipped',
        reason: shouldProbe ? undefined : 'probeData=false',
    };
    const coverageQuery = input.dateTable && input.dateColumn
    ...
    if (!query) {
        ...
        try {
            ...
            const expectedEvidenceRows = input.facts.filter((fact) => evidence.some((row) => row.tableName === fact.tableName && row.dateColumn === fact.dateColumn)).length;
            const parseShapeStatus = dateProofParseShapeStatus(diagnostics, evidence.length, eligibleFacts.length);
            ...
            const dateTableComplete = !coverageQuery || hasCompleteDateTableKeyProof(coverageEvidence.dateTable);
            const incompleteFacts = eligibleFacts
                .filter((fact) => !hasCompleteDateGrainProof(evidence.find((row) => row.tableName === fact.tableName && row.dateColumn === fact.dateColumn)))
                ...
                : parseShapeStatus
                    ...
                    : allRowsPresent
                        ? {
                            ...
                            expectedEvidenceRows: eligibleFacts.length,
                            queriedFacts: eligibleFacts.length,
                            reason: 'The probe returned a row for each requested column, but one or more numeric/date metrics did not parse (a probe/serialization defect, not an empty model). Report this and STOP; do not retry with probeData:false, run manual DAX, or mutate the model.',
                        ...
                        : {
                            ...
                            expectedEvidenceRows: eligibleFacts.length,
                            queriedFacts: eligibleFacts.length,
                            reason: 'The live DAX probe returned no parseable evidence for at least one requested fact date column.',
                            diagnostics,
                        };
        }
        catch (err) {
...
async function planDateTableForRead(input) {
    ...
    if (!query) {
        ...
        try {
            ...
            const diagnostics = daxResultDiagnostics(raw);
            const factEvidenceRows = eligibleFacts.filter((fact) => evidence.facts.some((row) => row.tableName === fact.tableName && row.dateColumn === fact.dateColumn)).length;
            const hasDateTableEvidence = evidence.dateTable !== undefined;
            const parsedEvidenceRows = dateTableCoverageEvidenceRows(evidence);
            const parseShapeStatus = dateProofParseShapeStatus(diagnostics, parsedEvidenceRows, 1 + eligibleFacts.length);
            ...
            const incompleteFacts = eligibleFacts
                .filter((fact) => !hasCompleteDateGrainProof(evidence.facts.find((row) => row.tableName === fact.tableName && row.dateColumn === fact.dateColumn)))
                .map((fact) => `${fact.tableName}[${fact.dateColumn}]`);
            const proofComplete = allRowsPresent && dateTableComplete && incompleteFacts.length === 0;
            probeStatus = proofComplete
                ? {
                ...
                : parseShapeStatus
                    ...
                    : allRowsPresent
                        ? {
                            ...
                            expectedEvidenceRows: 1 + eligibleFacts.length,
                            queriedFacts: eligibleFacts.length,
                            reason: 'The probe returned the date-table row and a row per fact, but one or more numeric/date metrics did not parse (a probe/serialization defect, not an empty model). Report this and STOP; do not retry with probeData:false, run manual DAX, or mutate the model.',
                            ...(dateTableComplete ? {} : { dateTableProofIncomplete: true }),
                            ...(incompleteFacts.length > 0 ? { incompleteFacts } : {}),
                            diagnostics,
                        }
...
function validateGovernedDateTableCreateInput(model, input) {
    const blockers = [];
...
function governedDateTableQuestions(input) {
    const questions = [];
    ...
    if (input.rangePolicy?.includes('future-horizon') &&
        (input.futureHorizonDays === undefined || input.futureHorizonDays <= 0)) {
        questions.push({
            id: 'future_horizon_days',
            prompt: 'How many days beyond the observed max fact date should the Date table extend, and why is that horizon required?',
    ...
    if (input.rangePolicy !== undefined &&
        ...
        questions.push({
            ...
            prompt: 'You supplied futureHorizonDays but selected a no-future-horizon range policy. Should the Date table include future dates?',
    ...
    if (input.refreshBeforeProbe === undefined) {
        questions.push({
            ...
            prompt: 'Should the live model be refreshed before probing fact date ranges? Choose true only when data/materialized calculated tables may be stale.',
            requiredFor: 'Live proof latency and freshness policy',
        });
    }
    return questions;
}
async function refreshLiveModelIfRequested(drv, mode, connection, enabled, phase, refreshType = 'Automatic') {
    if (!enabled)
        return { phase, skipped: true, reason: 'refreshBeforeProbe=false' };
    if (mode !== 'live')
        return { phase, skipped: true, reason: 'not-live' };
    try {
...
async function enforceGovernedDateCreateFactProof(drv, connection, model, facts) {
    ...
    const diagnostics = daxResultDiagnostics(raw);
    const missingFacts = eligibleFacts.filter((fact) => !evidence.some((row) => row.tableName === fact.tableName && row.dateColumn === fact.dateColumn));
    if (missingFacts.length > 0) {
        const parseShapeBlocker = dateProofParseShapeBlockedPayload(diagnostics, evidence.length, eligibleFacts.length);
        throwDateGateError(parseShapeBlocker
...
function buildGovernedDateTableExpression(input) {
    ...
    const factDatesExpression = factDateSets.length === 1
        ? firstFactDateSet
    ...
    const paddedMaxExpression = input.rangePolicy.includes('future-horizon') && futureHorizonDays > 0
        ? `__MaxDate + ${futureHorizonDays}`
        : '__MaxDate';
    const endExpression = input.rangePolicy === 'observed-full-years'
...
function governedDateTableColumnMetadata(tableName, dateColumn) {
    return [
        {
            tableName,
            name: dateColumn,
...
async function hardenGovernedDateTableColumnMetadata(drv, connection, tableName, dateColumn) {
    const results = [];
...
async function createGovernedDateTable(input) {
    ...
    if (questions.length > 0) {
        return {
            ...
            message: 'Date table creation refused until the calendar range policy is explicit. Do not guess literal dates or use TODAY()/NOW() anchors.',
        };
    }
    const rangePolicy = input.rangePolicy;
    const { mode, model, driver: drv, connection, } = await snapshotForWrite(input.folderPath, input.model, {
        includeMeasures: false,
        includeRoles: false,
    });
    if (mode !== 'live') {
        throwDateGateError('Governed Date table create refused: live mode is required.', {
            ...
            status: 'blocked',
            reason: 'not-live',
            message: 'This tool refreshes and proves the live model before marking the Date table. Folder-mode calendar writes cannot prove row coverage.',
        });
    }
    ...
    const refreshes = [];
    refreshes.push(await refreshLiveModelIfRequested(drv, mode, connection, input.refreshBeforeProbe === true, 'before-create', 'Automatic'));
    ...
    refreshes.push(await refreshLiveModelIfRequested(drv, mode, connection, true, 'after-create', 'Calculate'));
    const columnMetadataResults = await hardenGovernedDateTableColumnMetadata(drv, connection, input.tableName, input.dateColumn);
    const afterCreate = await readDriverSnapshot(drv, '(live)', { includeMeasures: false, includeRoles: false }, connection);
    const afterCreateWithSource = modelWithTrustedDateTableSource(afterCreate, input.tableName, expression);
    await enforceMarkAsDateGate(mode, afterCreateWithSource, connection, input.tableName, input.dateColumn, governedFacts, input.futureHorizonDays, rangePolicy === 'observed-full-years');
    ...
    try {
        ...
        dateTablePolicyStamp = {
            stamped: false,
            warning: 'Could not persist the governed Date-table range/horizon policy annotation (UNVERIFIED MS MCP write shape). Non-fatal; later steps may re-require futureHorizonDays until verified on a live model.',
            error: err instanceof Error ? err.message : String(err),
        };
    }
    const relationshipResults = [];
    if (input.createRelationships !== false) {
        const afterMark = await readDriverSnapshot(drv, '(live)', { includeMeasures: false, includeRoles: false }, connection);
        const afterMarkWithSource = modelWithTrustedDateTableSource(afterMark, input.tableName, expression);
        const activeDateRelationshipTables = new Set(afterMarkWithSource.relationships
        ...
        for (const fact of governedFacts) {
            const exists = afterMark.relationships.some((relationship) => relationship.fromTable === fact.tableName &&
            ...
            if (!check.valid) {
                throwDateGateError('Governed Date table relationship create refused: relationship check failed.', {
                    gate: 'governed-date-table-create',
                    status: 'blocked',
            ...
            await enforceDateRelationshipWriteGate('live', afterMarkWithSource, connection, candidate, 'create-date-relationship', {
                futureHorizonDays: input.futureHorizonDays,
                gate: 'governed-date-table-create',
                coverageFacts: governedFacts,
                allowCalendarEndAfterFactMax: rangePolicy === 'observed-full-years',
            });
            relationshipResults.push(await drv.createRelationship({
                fromTable: fact.tableName,
                fromColumn: fact.dateColumn,
                toTable: input.tableName,
                toColumn: input.dateColumn,
            ...
            if (isActive)
                activeDateRelationshipTables.add(fact.tableName);
        }
    }
    const finalSnapshot = await readDriverSnapshot(drv, '(live)', { includeMeasures: false, includeRoles: false }, connection);
    const finalSnapshotWithSource = modelWithTrustedDateTableSource(finalSnapshot, input.tableName, expression);
    ...
    const coverageEvidence = coverageQuery && coverageProbeError === undefined
        ? parseDateTableCoverageProbeResult(rawCoverage)
        : { facts: [] };
    const coverageEvidenceRows = dateTableCoverageEvidenceRows(coverageEvidence);
    const coverageDiagnostics = coverageQuery && coverageProbeError === undefined
        ? daxResultDiagnostics(rawCoverage)
        : undefined;
    const coverageParseShapeBlocker = coverageDiagnostics !== undefined
        ? dateProofParseShapeBlockedPayload(coverageDiagnostics, coverageEvidenceRows, 1 + governedFacts.length)
        : undefined;
    const coverage = planDateTableCoverage(finalSnapshotWithSource, {
        dateTable: input.tableName,
        dateColumn: input.dateColumn,
        facts: governedFacts,
        futureHorizonDays: input.futureHorizonDays,
...
function modelWithTrustedDateTableSource(model, tableName, expression) {
    return {
        ...model,
        tables: model.tables.map((table) => table.name === tableName
            ? {
                ...table,
                expression,
...
function quoteDaxTable(name) {
    return `'${name.replace(/'/g, "''")}'`;
}
function daxColumnRef(tableName, columnName) {
...
function eligibleDateFacts(model, facts) {
...
async function enforceDateRelationshipWriteGate(mode, model, connection, candidate, requiredAction, gateOptions = 0) {
    ...
    const governedDatePolicy = readGovernedDatePolicy(model.tables.find((table) => table.name === candidate.toTable));
    const requestedFutureHorizonDays = typeof gateOptions === 'number' ? gateOptions : gateOptions.futureHorizonDays;
    ...
    if (futureHorizonDays > MAX_FUTURE_HORIZON_DAYS) {
        throwDateGateError(`Date relationship write refused: futureHorizonDays exceeds ${MAX_FUTURE_HORIZON_DAYS}.`, {
            gate: typeof gateOptions === 'number'
    ...
    const gate = typeof gateOptions === 'number'
        ? 'date-grain-write-gate'
        : (gateOptions.gate ?? 'date-grain-write-gate');
    const coverageFacts = typeof gateOptions === 'number' ? undefined : gateOptions.coverageFacts;
    const allowCalendarEndAfterFactMax = (typeof gateOptions === 'number' ? undefined : gateOptions.allowCalendarEndAfterFactMax) ??
        governedDatePolicy.allowCalendarEndAfterFactMax ??
        false;
    if (!looksLikeDateRelationshipCandidate(model, candidate))
        return [];
    ...
    if (!looksLikeDateRelationshipEndpoint(model, candidate.toTable, candidate.toColumn)) {
        throwDateGateError('Date relationship write refused: target date endpoint is not a governed temporal date column (missing/non-date column, or an auto LocalDateTable).', {
            gate,
            status: 'blocked',
    ...
    if (effectiveCrossFiltering !== 'single') {
        throwDateGateError('Date relationship write refused: active date relationships must use single-direction cross filtering.', {
            gate,
            status: 'blocked',
    ...
    const diagnostics = daxResultDiagnostics(raw);
    const coverageParseShapeBlocker = dateProofParseShapeBlockedPayload(diagnostics, dateTableCoverageEvidenceRows(coverageEvidence), 1 + requiredFacts.length);
    if (coverageParseShapeBlocker) {
        throwDateGateError('Date relationship write refused: Date-table proof result shape was not recognized.', {
            gate,
            status: 'blocked',
            candidate,
    ...
    const coverage = planDateTableCoverage(model, {
        dateTable: candidate.toTable,
        dateColumn: candidate.toColumn,
        facts: requiredFacts,
        futureHorizonDays,
    ...
    const grainParseShapeBlocker = dateProofParseShapeBlockedPayload(diagnostics, evidence.length, requiredFacts.length);
    if (grainParseShapeBlocker) {
        throwDateGateError('Date relationship write refused: date-grain proof result shape was not recognized.', {
            gate,
            status: 'blocked',
            candidate,
            evidenceRows: evidence.length,
    ...
    const plan = planDateGrain(model, {
        facts: requiredFacts,
        dateTable: candidate.toTable,
        dateColumn: candidate.toColumn,
    ...
    const factPlan = plan.facts.find((candidatePlan) => candidatePlan.tableName === fact.tableName && candidatePlan.dateColumn === fact.dateColumn);
    const matchingWrite = factPlan?.writePlan.some((item) => {
        ...
        return (item.action === 'create-date-relationship' &&
    ...
    const inactiveCreateSafe = requiredAction === 'create-date-relationship' &&
    ...
    if (factPlan?.observedGrain !== 'day' || (matchingWrite !== true && !inactiveCreateSafe)) {
        throwDateGateError(`Date relationship write refused: observed grain is "${factPlan?.observedGrain ?? 'unknown'}", not proven daily with a matching planner write.`, {
            gate,
            status: 'blocked',
...
function looksLikeDateRelationshipCandidate(model, candidate) {
...
function looksLikeDateRelationshipEndpoint(model, tableName, columnName) {
    const table = model.tables.find((candidate) => candidate.name === tableName);
    const column = findModelColumn(model, tableName, columnName);
    if (!table || !column || table.isAutoDateTable)
        return false;
...
function findModelColumn(model, tableName, columnName) {
    return model.tables
        .find((table) => table.name === tableName)
        ?.columns.find((column) => column.name === columnName);
}
...
function throwDateGateError(message, report) {
    const err = new Error(message);
    err.report = report;
    throw err;
}
function throwInputValidationError(message, report) {
    const err = new Error(message);
    err.report = { gate: 'tool-input-validation', status: 'blocked', ...report };
    throw err;
}
function enforceMeasureIntentForWrite(toolName, measureName, expression, measureIntent, model) {
    if (!measureIntent) {
    ...
    const missingRefs = intent.sourceRefs.filter((ref) => ref.kind === 'column'
        ? findColumn(fieldIndex, ref.table, ref.column) === null
        : findMeasure(fieldIndex, ref.table, ref.column) === null);
    if (missingRefs.length > 0) {
        throwInputValidationError('Measure write refused: measureIntent sourceRefs are not in the model.', {
            tool: toolName,
    ...
    if (expression && expressionUsesTimeIntelligence(expression)) {
        if (!hasMeasureIntentTimeEvidence(intent)) {
            throwInputValidationError('Measure write refused: time-intelligence DAX requires confirmed timeIntelligence evidence.', {
                tool: toolName,
        ...
        const missingDateRefs = time.dateRefs.filter((ref) => findColumn(fieldIndex, ref.table, ref.column) === null);
        if (missingDateRefs.length > 0) {
            throwInputValidationError('Measure write refused: timeIntelligence dateRefs are not in the model.', {
                tool: toolName,
        ...
        if (findColumn(fieldIndex, time.dateTable, time.dateColumn) === null) {
            throwInputValidationError('Measure write refused: timeIntelligence dateTable/dateColumn is not in the model.', {
                tool: toolName,
...
function hasMeasureIntentTimeEvidence(intent) {
    const time = intent.timeIntelligence;
...
async function enforceTimeIntelligenceNotBlankInDefaultContext(connection, model, toolName, measureName, expression, intent) {
    ...
    if (bare.extraArgs.length > 0) {
        const soleYearEnd = bare.period === 'YTD' &&
...
function baseExpressionReferencesTable(baseExpression, tableName) {
...
function calcExpressionReferenceAdvisory(model, hostTable, expression) {
...
function enforceProvenRelationshipIdentity(relationship, operation) {
    if (relationship.identityProven !== true) {
        throwDateGateError(`Refused: relationship "${relationship.id}" does not have a proven model identity for ${operation}.`, {
...
function enforceNoDirectDateTableMetadataWrite(model, input) {
    if (input.kind === 'table') {
        if (isTimeDataCategory(input.dataCategory)) {
            throwDateGateError('Direct table dataCategory:"Time" writes are refused. Use pbi_table_mark_as_date with date-table coverage facts so the live continuity/coverage gate runs.', {
    ...
    const marksDateTable = isTimeDataCategory(table?.dataCategory) || isTimeDataCategory(effectiveColumnCategory);
    const touchesDateMetadata = input.dataType !== undefined || input.isKey !== undefined || input.dataCategory !== undefined;
    const wouldSetTimeCategory = isTimeDataCategory(input.dataCategory);
    const wouldCreateMarkedDateKey = touchesDateMetadata &&
        effectiveIsKey &&
        isTemporalDataType(effectiveDataType) &&
        marksDateTable;
    const wouldMutateMarkedDateKeyExpression = input.expression !== undefined && effectiveIsKey && marksDateTable;
    if (wouldSetTimeCategory || wouldCreateMarkedDateKey || wouldMutateMarkedDateKeyExpression) {
        throwDateGateError('Direct Date-table key/category writes are refused. Use pbi_table_mark_as_date with date-table coverage facts so the live continuity/coverage gate runs.', {
            gate: 'direct-date-table-metadata-gate',
            status: 'blocked',
...
function enforceNoVolatileCalendarTableCreate(input) {
    ...
    if (looksLikeCalendarTableCreate(input)) {
        throwDateGateError('Date table creation refused: generic table creation cannot prove calendar coverage. Use pbi_model_plan_date_table against an existing governed Date table before relying on Date fields, and do not create calendar bounds from prompt judgment.', {
            gate: 'date-table-create-gate',
            status: 'blocked',
...
async function enforceMarkAsDateGate(mode, model, connection, tableName, dateColumn, facts, futureHorizonDays, allowCalendarEndAfterFactMax = false) {
    const table = model.tables.find((candidate) => candidate.name === tableName);
    ...
    if (!table || !column || table.isAutoDateTable || !isTemporalType(column.dataType)) {
        throwDateGateError('Mark-as-date refused: table/column metadata is not a valid governed date key.', {
    ...
    const evidence = parseDateTableCoverageProbeResult(raw);
    const diagnostics = daxResultDiagnostics(raw);
    const parseShapeBlocker = dateProofParseShapeBlockedPayload(diagnostics, dateTableCoverageEvidenceRows(evidence), 1 + requiredFacts.length);
...
tool('pbi_model_snapshot', 'Read Live Model', 'Connect to the model (live Power BI Desktop instance, else folder fallback) and return the full semantic model: tables, columns, measures, relationships. Read-only.', { folderPath: MODEL_FOLDER_FIELD, model: MODEL_SELECT_FIELD }, { readOnlyHint: true, idempotentHint: true }, async (input) => {
...
tool('pbi_model_list_tables', 'List Tables', 'List table inventory in the model (live Desktop instance, else folder). Default live mode uses a fast table-only read and returns table-level metadata, with counts when available; opt into full nested field payload only when needed.', {
...
tool('pbi_data_dictionary_get', 'Get Data Dictionary', 'Return a read-only canonical data dictionary projection from semantic model metadata only: tables, fields, measures, relationships, counts, and Table[Field] refs. This is model metadata, not business meaning; it never reads rows, executes DAX, or infers formulas, KPIs, ownership, or business definitions.', {
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
    includeHidden: z
        .boolean()
    ...
    includeNested: z
        .boolean()
        .optional()
        .describe('Include duplicate nested fields/measures inside each table. Defaults to false; top-level fields/measures are always returned.'),
    tableNames: z
        .array(z.string())
        .optional()
    ...
    refs: z
        .array(z.string())
        .optional()
        .describe('Optional exact canonical ref filter such as Table[Field] for columns or measures.'),
}, { readOnlyHint: true, destructiveHint: false, idempotentHint: true }, async (input) => {
    const snapshot = await snapshotModel(input.folderPath, input.model, {
        includeMeasures: true,
        includeRoles: false,
    });
...
tool('pbi_model_plan_star_schema_join', 'Plan Star-Schema Join', 'Return a deterministic star-schema/shared-dimension plan for a cross-fact join or TREATAS/MOD009/MOD010 remediation. Read-only: this tool does not write anything. It returns the proposed design, blockers, dimensions, key-column writes, relationship create writes, relationship repair writes, hide-FK writes, and whether a direct fact relationship is allowed. If the requested tables also expose shared temporal axes, the plan includes dateAxisRequirement: one governed Date table shared by every participating fact is required; LocalDateTable relationships are not a conformed report axis, so use pbi_date_table_create_governed rather than manual Date-table or relationship steps.', {
    ...
    axes: z
        .array(z.string())
        .optional()
        .describe('Optional shared axes/key column names to evaluate. If omitted, the planner considers shared column names from the model snapshot.'),
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { readOnlyHint: true, destructiveHint: false, idempotentHint: true }, async (input) => {
    const { mode, model, connection } = await snapshotModel(input.folderPath, input.model);
...
tool('pbi_model_plan_actuals_targets_join', 'Plan Actuals/Targets Join', 'Return a deterministic read-only actuals/targets join readiness plan that combines star-schema shared-dimension planning with batched date-grain proof before asking the user any grain question. It routes temporal axes to date-grain planning, routes non-temporal axes to star-schema planning, and requires one governed Date table shared by every participating fact before reporting ready. Auto LocalDateTable relationships are not a conformed actuals/targets date axis; when dateAxisRequirement says governed-date-table-required, use pbi_date_table_create_governed for the Date table and relationships. Ask only for unobservable business policy such as allocation or missing-target behavior.', {
    ...
    axes: z
        .array(z.string())
        .optional()
        .describe('Optional shared axes/key column names to evaluate. Temporal axes are routed to date-grain planning; non-temporal axes are routed to star-schema shared dimensions. If omitted, the tool discovers same-name shared columns from metadata.'),
    dateRefs: z
        .array(z.object({
        ...
        .optional()
        .describe('Optional explicit actual/target date columns. Use only when metadata has no same-name shared temporal column or multiple date roles require a specific role.'),
    dateTable: z
        .string()
        .optional()
...
tool('pbi_model_apply_star_schema_join', 'Apply Star-Schema Join', 'Batch-apply or dry-run a deterministic planner-backed shared-dimension star-schema remediation to a live Power BI Desktop model. Requires explicit axes, re-plans from the targeted live model, refuses planner blockers, creates/reuses calculated dimensions, refreshes calculated metadata, hardens dimension key data types, configures keys, repairs/creates single-direction many-to-one relationships, hides fact-side FK fields, and validates the final state. If the model also has shared temporal axes, the response carries dateAxisRequirement and is date-axis incomplete until one governed Date table is shared by every participating fact; LocalDateTable relationships are not sufficient, so use pbi_date_table_create_governed for the Date table and Date relationships.', {
    ...
    axes: z
        .array(z.string())
        .min(1)
        .describe('Explicit shared axes/key column names to apply. Required for writes so accidental same-name columns are not remediated silently.'),
    dryRun: z
        .boolean()
        .optional()
    ...
    runModelCheck: z
        .boolean()
        .optional()
        .describe('Optional. When true, also returns pbi_model_check/modelDoctor output after targeted validation passes.'),
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { destructiveHint: false, idempotentHint: false }, (input) => applyStarSchemaJoin(input));
tool('pbi_model_plan_date_grain', 'Plan Date Grain', 'Return a deterministic date-grain plan for fact date columns before activating date relationships, simplifying target/actual measures, or before asking the user to choose target grain/day/month/year. In live mode it runs one generated read-only DAX probe for all requested facts; in folder mode it fails closed with metadata-only guidance. Use observedGrain/probeStatus as the evidence for observable date grain. If proof fails, is incomplete, or returns parse-shape-unrecognized/evidenceRows:0 from the ROW()-based proof, report the structured blocker/status and STOP before Date relationship activation or measure rewrite; do not claim the model is empty, use pbi_dax_query/manual DAX/pbi_model_refresh, retry with probeData:false, or fall back to primitive Date/relationship writes. Ask the user only for business semantics the model cannot prove, such as allocation or missing-target behavior. It also reports auto date tables so agents can avoid slow, repetitive LocalDateTable inventories.', {
    facts: z
        .array(z.object({
        ...
        .min(1)
        .describe('Fact date columns to probe. Batch related actual/target facts here so the tool can use one live DAX query.'),
    dateTable: z
        .string()
        .optional()
...
tool('pbi_model_plan_date_table', 'Plan Date Table Coverage', 'Return a deterministic coverage plan for a governed Date table before editing calendar bounds, marking a date table, disabling auto date/time, or relying on Date-table fields for target/actual analysis. In live mode it runs one read-only DAX proof over the date table and the complete model-derived fact-date coverage set. It blocks volatile TODAY()/NOW()/M current-date anchors, literal hardcoded calendar bounds, blanks, duplicate date keys, gaps, and Date tables that do not cover observed fact min/max dates. Date ranges extending beyond observed fact max dates require explicit futureHorizonDays. A blocked, non-succeeded, or parse-shape-unrecognized proof is not actionable coverage evidence; report it and STOP with no calendar edit, mark-as-date, pbi_dax_query/manual DAX/pbi_model_refresh fallback, probeData:false retry, or primitive Date write.', {
    ...
    facts: z
        ...
        .min(1)
        .describe('Seed fact date columns to prove coverage against. The tool expands this list to every model-derived fact-date column that must be covered.'),
    futureHorizonDays: FUTURE_HORIZON_DAYS_FIELD,
    probeData: z
        .boolean()
        .optional()
        .describe('Defaults to true. In live mode, run one exact read-only DAX proof. Set false only for an initial metadata-only plan; writes must not rely on skipped proof, and probeData:false is forbidden as recovery after failed, incomplete, or parse-shape proof.'),
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { readOnlyHint: true, destructiveHint: false, idempotentHint: true }, (input) => planDateTableForRead(input));
tool('pbi_date_table_create_governed', 'Create Governed Date Table', 'Create, optionally refresh with explicit refreshBeforeProbe authorization, prove, harden metadata, mark, and optionally relate a governed Date table in the live model. This is the only write path for new Date/Calendar tables. It asks for clarifyingQuestions instead of writing when calendar policy or refreshBeforeProbe policy is ambiguous, generates dynamic fact-anchored DAX from supplied date columns, never uses literal guessed DATE bounds or TODAY()/NOW(), never refreshes before proof unless refreshBeforeProbe is explicitly true, writes explicit generated-column dataType/summarize/sort metadata, marks the table as a Date table, and creates single-direction many-to-one fact date relationships when requested. Import models may reject the column isKey write; table dataCategory:Time plus live unique/continuous Date-key proof is accepted. For multiple date roles on one fact table, the first role is active and later roles are inactive. If proof blocks or returns proof-parse-shape-unrecognized/evidenceRows:0 from the ROW()-based proof, report that blocker verbatim and STOP; do not claim the model is empty, use pbi_dax_query/manual DAX/pbi_model_refresh, retry with probeData:false, or reconstruct the Date table through pbi_table_create/pbi_column_update/pbi_relationship_create primitives.', {
    ...
    facts: z
        ...
        .min(1)
        .describe('Fact date columns that must be covered. Pass all actual/target/budget/forecast date columns involved in the analysis.'),
    rangePolicy: z
        .enum([
        'observed-min-max',
        'observed-full-years',
        ...
        .optional()
        .describe('Explicit calendar policy. Omit to get clarifyingQuestions instead of a write. No silent default is allowed.'),
    futureHorizonDays: FUTURE_HORIZON_DAYS_FIELD,
    refreshBeforeProbe: z
        .boolean()
        .optional()
        .describe('Explicit pre-proof refresh policy. If omitted, the tool returns needs-user-input. Set true only when refresh is authorized; set false to prove without pre-probe refresh.'),
    createRelationships: z
        .boolean()
        .optional()
        .describe('Defaults to true. Create single-direction many-to-one relationships from each fact date column to the new Date table; for multiple date roles on one fact table, the first role is active and later roles are inactive.'),
    description: z.string().optional().describe('Optional table description.'),
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { destructiveHint: false, idempotentHint: false }, (input) => createGovernedDateTable(input));
tool('pbi_model_refresh', 'Refresh Live Model', 'Refresh/process the connected live Power BI Desktop semantic model through the Microsoft Modeling MCP only when the user explicitly authorizes processing/import refresh. Requires confirmReprocess:true (an explicit per-call authorization) because a refresh re-processes Import data and can change every measure value and probe result. Use this instead of asking the user to click Refresh when a modeling workflow needs Import data materialized. Never use this as a fallback after proof-parse-shape-unrecognized, parse-shape-unrecognized/evidenceRows:0 from a ROW()-based Date proof, or any Date proof blocker that instructs STOP. Folder mode is refused because refresh is a live engine operation.', {
    refreshType: z
    ...
    confirmReprocess: z
        .boolean()
        .optional()
        .describe('Must be explicitly true to authorize the reprocess. A refresh re-materializes Import data and can change measure values and probe results, so it is gated behind this per-call consent rather than being callable as a silent fallback.'),
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { destructiveHint: false, idempotentHint: true }, async (input) => {
    ...
    if (input.confirmReprocess !== true) {
        throwInputValidationError('Model refresh refused: a reprocess re-materializes Import data and can change measure values, so it requires explicit authorization. Re-call with confirmReprocess:true only when the user has approved processing/import refresh.', {
...
tool('pbi_model_list_columns', 'List Columns', 'List the columns in the model (live Desktop instance, else folder). Read-only.', { folderPath: MODEL_FOLDER_FIELD, model: MODEL_SELECT_FIELD }, { readOnlyHint: true, idempotentHint: true }, async (input) => {
    const { mode, model } = await snapshotModel(input.folderPath, input.model, {
        includeMeasures: false,
        includeRoles: false,
    });
...
tool('pbi_model_list_measures', 'List Measures', 'List the measures in the model (live Desktop instance, else folder). Read-only.', { folderPath: MODEL_FOLDER_FIELD, model: MODEL_SELECT_FIELD }, { readOnlyHint: true, idempotentHint: true }, async (input) => {
    const { mode, model } = await snapshotModel(input.folderPath, input.model, {
        includeMeasures: true,
        includeRoles: false,
    });
...
tool('pbi_measure_update', 'Update Measure (DAX-gated)', 'Update an existing measure. Confirmed measure intent is required for every update, including metadata-only changes. If a new expression is supplied, time-intelligence DAX requires confirmed Date policy evidence and the expression passes the same in-code DAX-reference gate as create.', {
    ...
    enforceMeasureIntentForWrite('pbi_measure_update', input.name, input.expression, input.measureIntent, model);
    let blankRiskWarning;
    if (input.expression !== undefined) {
        blankRiskWarning = await enforceTimeIntelligenceNotBlankInDefaultContext(connection, model, 'pbi_measure_update', input.name, input.expression, input.measureIntent);
        const check = daxReferenceCheck(input.expression, model, { hostTable: input.tableName });
        if (!check.valid) {
...
function unresolvedRefCount(expression, model, hostTable) {
...
function expressionDependsOnTarget(expression, fullModel, modelWithoutTarget, hostTable) {
    if (!expression)
        return false;
...
function modelWithoutTable(model, tableName) {
    return {
        ...model,
        tables: model.tables.filter((table) => table.name !== tableName),
        relationships: model.relationships.filter((rel) => rel.fromTable !== tableName && rel.toTable !== tableName),
    };
}
function modelWithoutColumn(model, tableName, columnName) {
    return {
        ...model,
        tables: model.tables.map((table) => table.name === tableName
...
function modelWithoutMeasure(model, tableName, measureName) {
    return {
        ...model,
        tables: model.tables.map((table) => table.name === tableName
...
function dependentExpressionWarnings(fullModel, pruned, label) {
    const warnings = [];
...
function computeTableDeleteDependencyWarnings(model, tableName) {
    try {
        const warnings = [];
        for (const rel of model.relationships) {
            if (rel.fromTable === tableName || rel.toTable === tableName) {
                warnings.push(`relationship "${rel.id}" (${rel.fromTable}[${rel.fromColumn}] -> ${rel.toTable}[${rel.toColumn}]) uses this table`);
            }
        }
        const pruned = modelWithoutTable(model, tableName);
        warnings.push(...dependentExpressionWarnings(model, pruned, (kind, table, name) => `${kind} "${table}[${name}]" references this table`));
        return warnings;
    }
    catch {
        return [];
...
function computeColumnDeleteDependencyWarnings(model, tableName, columnName) {
    try {
        const warnings = [];
        for (const rel of model.relationships) {
            ...
            if (isFrom || isTo) {
                warnings.push(`relationship "${rel.id}" (${rel.fromTable}[${rel.fromColumn}] -> ${rel.toTable}[${rel.toColumn}]) uses this column`);
            }
        }
        const pruned = modelWithoutColumn(model, tableName, columnName);
        warnings.push(...dependentExpressionWarnings(model, pruned, (kind, table, name) => `${kind} "${table}[${name}]" references this column`));
        return warnings;
    }
    catch {
        return [];
...
function computeMeasureDeleteDependencyWarnings(model, tableName, measureName) {
    try {
        const pruned = modelWithoutMeasure(model, tableName, measureName);
        return dependentExpressionWarnings(model, pruned, (kind, table, name) => `${kind} "${table}[${name}]" references this measure`);
    }
    catch {
        return [];
    }
}
function computeRelationshipDeleteDependencyWarnings(model, relationship) {
    try {
        const warnings = [];
        if (relationship.isActive) {
            warnings.push(`relationship is active; filter propagation between ${relationship.fromTable} and ${relationship.toTable} will be lost`);
        }
        if (relationship.crossFilteringBehavior === 'both') {
            warnings.push(`relationship is bidirectional; cross-filtering between ${relationship.fromTable} and ${relationship.toTable} will be lost`);
        }
        return warnings;
    }
    catch {
...
tool('pbi_measure_delete', 'Delete Measure', 'Delete a measure from the connected model.', {
    tableName: z.string().describe('Home table of the measure.'),
    name: z.string().describe('Measure name to delete.'),
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { destructiveHint: true, idempotentHint: false }, async (input) => {
    const { mode, model, driver: drv, connection, } = await snapshotForWrite(input.folderPath, input.model, {
        includeMeasures: true,
        includeRoles: false,
    });
...
tool('pbi_table_create', 'Create Table', 'Create a non-Date table on the connected model. Pass `mExpression` for a Power Query (M) partition, or `expression` for a DAX calculated table (for example, SUMMARIZE/SELECTCOLUMNS/ADDCOLUMNS). A calculated-table expression defines the table AND its columns in its own row context, so it is validated by the modeling engine on create (invalid DAX is rejected with a clear error and nothing is written) — no pre-flight reference gate runs. HARD GATE: Date/calendar table creation is refused through this primitive tool, including dynamic-looking calculated sources; use pbi_date_table_create_governed for new Date/Calendar tables and pbi_model_plan_date_table for existing Date-table coverage checks. Do not manually replay cross-fact/shared-dimension star-schema table writes with this primitive tool; use pbi_model_apply_star_schema_join with explicit axes for cross-fact joins. On success in live mode the table appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata. In folder mode, call pbi_model_export.', {
    ...
    expression: z
        .string()
        .optional()
        .describe('DAX expression for a non-Date calculated table. Defines the table and its columns; validated by the engine on create.'),
    description: z.string().optional().describe('Optional table description.'),
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { destructiveHint: false, idempotentHint: false }, async (input) => {
    if ((input.mExpression === undefined) === (input.expression === undefined)) {
        throwInputValidationError('Table create refused: supply exactly one source expression, either mExpression or expression.', {
...
tool('pbi_table_update', 'Update Table', 'Update an existing table: rename, edit description, or toggle visibility. Direct `dataCategory:"Time"` writes are REFUSED because they bypass the Date-table continuity/coverage gate; use pbi_table_mark_as_date with fact coverage inputs. On success in live mode the change appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata. In folder mode, call pbi_model_export.', {
    ...
    dataCategory: z
        ...
        .describe('Table semantic category. `"Time"` is refused here; use pbi_table_mark_as_date with fact coverage inputs so the Date-table gate runs.'),
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { destructiveHint: false, idempotentHint: true }, async (input) => {
    const { mode, model, driver: drv, connection, } = await snapshotForWrite(input.folderPath, input.model, {
...
tool('pbi_table_mark_as_date', 'Mark As Date Table', 'Mark a table as the model date table so time-intelligence (DATEADD/SAMEPERIODLASTYEAR/TOTALYTD, etc.) works. HARD GATE: the chosen date key must be proven live as a continuous unique daily date/dateTime column with no blanks, duplicates, gaps, or auto-date table target, and the Date table must cover observed min/max dates for the complete model-derived fact-date coverage set, not just the caller-supplied sample. Volatile current-date anchors and literal hardcoded calendar bounds are refused. Folder-mode marking is refused because coverage cannot be proven. If coverage/key proof blocks or returns proof-parse-shape-unrecognized, report the blocker and STOP; do not use primitive table/column updates. Sets the table `dataCategory` to "Time" and attempts `isKey` on the given date column; Import models may reject the `isKey` write, in which case the live proof remains authoritative. Idempotent — re-marking an already-marked table is a no-op. On success in live mode the change appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata.', {
    ...
    facts: z
        ...
        .min(1)
        .describe('Seed fact date columns that the governed Date table must cover. Required so the live coverage gate can expand to the complete model-derived coverage set and prove observed min/max before marking.'),
    futureHorizonDays: FUTURE_HORIZON_DAYS_FIELD,
    folderPath: MODEL_FOLDER_FIELD,
    model: MODEL_SELECT_FIELD,
}, { destructiveHint: false, idempotentHint: true }, async (input) => {
    ...
    const markGateWarnings = await enforceMarkAsDateGate(mode, model, connection, input.tableName, input.dateColumn, input.facts, input.futureHorizonDays);
    const result = await drv.markAsDateTable(input.tableName, input.dateColumn, connection);
    const readMarkVerification = () => readDriverSnapshot(drv, mode === 'live' ? '(live)' : undefined, { includeMeasures: false, includeRoles: false }, connection);
...
tool('pbi_table_delete', 'Delete Table', 'Delete a table (and its columns/measures) from the connected model.', {
...
tool('pbi_column_create', 'Create Column', 'Create a column on a table. Pass `sourceColumn` for a data column, or `expression` for a DAX calculated column. Calculated columns (via `expression`) are supported on imported (Power Query / M) tables too — not just calculated tables. A calculated-column expression runs in the host table’s row context and is validated by the modeling engine on create (invalid DAX is rejected with a clear error and nothing is written) — no pre-flight reference gate runs. On success in live mode the column appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata. In folder mode, call pbi_model_export.', {
    ...
    expression: z
        .string()
        .optional()
        .describe('DAX expression for a calculated column (runs in the host table’s row context). Validated by the engine on create.'),
    formatString: z.string().optional().describe('Format string (bare TMDL backslash form).'),
    sortByColumn: z
        .string()
        .optional()
    ...
    isKey: z
        ...
        .describe('Mark this column as the table primary key. Date-table key/category writes are refused here; use pbi_table_mark_as_date with fact coverage inputs.'),
    dataCategory: z
        .string()
        .optional()
        // UNVERIFIED: MS-MCP `dataCategory` write key inferred (see ColumnWrite in model-driver).
        .describe('Semantic data category (e.g. "City", "Country", "Latitude", "Longitude"). Geo categories drive map visuals.'),
    ...
    if ((input.sourceColumn === undefined) === (input.expression === undefined)) {
        throwInputValidationError('Column create refused: supply exactly one source path, either sourceColumn or expression.', {
    ...
    const referenceAdvisory = calcExpressionReferenceAdvisory(model, input.tableName, input.expression);
    return {
        created: true,
        mode,
        persist: mode === 'live' ? LIVE_MODEL_PERSISTENCE : FOLDER_MODEL_PERSISTENCE,
...
tool('pbi_column_update', 'Update Column', 'Update an existing column: rename, change data type/format/summarizeBy/visibility, or edit a calculated column expression. Date-table key/category writes are REFUSED here because they bypass the Date-table continuity/coverage gate; use pbi_table_mark_as_date with fact coverage inputs. A new `expression` (calculated column) is validated by the modeling engine on update (invalid DAX is rejected and nothing is written). On success in live mode the change appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata. In folder mode, call pbi_model_export.', {
    ...
    isKey: z
        ...
        .describe('Mark/unmark this column as the table primary key. Marking a date-table key is refused here; use pbi_table_mark_as_date with fact coverage inputs.'),
    dataCategory: z
        .string()
        .optional()
...
tool('pbi_column_delete', 'Delete Column', 'Delete a column from a table on the connected model.', {
    tableName: z.string().describe('Table that hosts the column.'),
...
tool('pbi_relationship_create', 'Create Relationship (validity-gated)', 'Create a relationship between two columns. HARD GATES: an in-code relationship check runs first against the live model; temporal Date relationship creates also require a governed marked Date endpoint, live Date-table coverage proof, live date-only day-grain proof, and single-direction filtering. Sparse fact dates are valid; Date-key continuity is proven on the Date table. Active temporal creates require a matching planner write; inactive role-playing Date relationships are allowed after proof. If an endpoint is missing, key types mismatch, an active path would become ambiguous, Date coverage/grain is not proven, proof parsing is unrecognized, or the temporal create is bidirectional, the create is REFUSED and nothing is written. Temporal proof failure means STOP and report the blocker; do not replay manual relationship writes. Do not manually replay cross-fact/shared-dimension star-schema relationship writes with this primitive tool; use pbi_model_apply_star_schema_join with explicit axes for cross-fact joins. Cardinality defaults to manyToOne and is sent to the modeling engine as fromCardinality/toCardinality. Bidirectional cross-filtering is allowed for non-date relationships but returned as an advisory warning. On success in live mode the relationship appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata. In folder mode, call pbi_model_export.', {
    ...
    if (!check.valid) {
        const err = new Error(`Refused: relationship ${input.fromTable}[${input.fromColumn}] → ${input.toTable}[${input.toColumn}] is not valid for the model.`);
        err.report = {
            gate: 'relationship-check',
            blocking: check.blocking,
            warnings: check.warnings,
    ...
    const dateTableWarnings = await enforceDateRelationshipWriteGate(mode, model, connection, {
        fromTable: input.fromTable,
        fromColumn: input.fromColumn,
        toTable: input.toTable,
        toColumn: input.toColumn,
...
tool('pbi_relationship_update', 'Update Relationship (validity-gated)', 'Update an existing relationship by id: re-point endpoints, change cardinality, change cross-filtering, or toggle active. The same in-code relationship check as create runs first (the edited row is excluded from the active-path ambiguity check). Activating/re-pointing an active temporal Date relationship, or changing its shape, also requires a governed marked Date endpoint, live Date-table coverage proof, live date-only day-grain proof, and single-direction filtering. Sparse fact dates are valid; Date-key continuity is proven on the Date table. On a structural/date-coverage/date-grain/parse-shape violation the update is REFUSED. Temporal proof failure means STOP and report the blocker; do not replay manual relationship writes. Bidirectional cross-filtering is allowed for non-date relationships but returned as an advisory warning. On success in live mode the change appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata. In folder mode, call pbi_model_export.', {
    ...
    enforceProvenRelationshipIdentity(existing, 'update');
    {
        ...
        if (!check.valid) {
            const err = new Error(`Refused: updated relationship "${input.id}" is not valid for the model.`);
            err.report = {
                gate: 'relationship-check',
                blocking: check.blocking,
                warnings: check.warnings,
        ...
        warnings = [...check.warnings];
        const endpointChanged = candidate.fromTable !== existing.fromTable ||
            candidate.fromColumn !== existing.fromColumn ||
            candidate.toTable !== existing.toTable ||
            candidate.toColumn !== existing.toColumn;
        const cardinalityChanged = input.cardinality !== undefined && input.cardinality !== existing.cardinality;
        const dateShapeChanged = candidate.crossFilteringBehavior !== existing.crossFilteringBehavior ||
            candidate.isActive !== existing.isActive ||
            cardinalityChanged;
        dateTableWarnings = await enforceDateRelationshipWriteGate(mode, model, connection, {
            id: existing.id,
...
tool('pbi_relationship_activate', 'Activate Relationship', 'Mark a relationship active by id. HARD GATE: temporal Date relationship activation requires a governed marked Date endpoint, live Date-table coverage proof, live date-only day-grain proof with observedGrain "day", a matching activate writePlan item, and single-direction filtering. Sparse fact dates are valid; Date-key continuity is proven on the Date table. If coverage/grain proof blocks or proof parsing is unrecognized, activation is REFUSED and nothing is written. Temporal proof failure means STOP and report the blocker; do not replay manual relationship writes. On success in live mode the change appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata. In folder mode, call pbi_model_export.', {
    ...
    }, model, { ignoreRelationshipId: input.id });
    if (!check.valid) {
        const err = new Error(`Refused: activating relationship "${input.id}" is not valid for the model.`);
        err.report = {
            gate: 'relationship-check',
            blocking: check.blocking,
            warnings: check.warnings,
    ...
    if (existing && !existing.isActive) {
        dateTableWarnings = await enforceDateRelationshipWriteGate(mode, model, connection, {
            ...
            isActive: true,
            crossFilteringBehavior: existing.crossFilteringBehavior,
        }, 'activate-date-relationship', input.futureHorizonDays);
    }
...
tool('pbi_relationship_deactivate', 'Deactivate Relationship', 'Mark a relationship inactive by id. On success in live mode the change appears in Desktop immediately; press Ctrl+S to persist live semantic-model metadata. In folder mode, call pbi_model_export.', {
    ...
    const targetsDate = (relationship) => isTemporalDataType(findModelColumn(model, relationship.toTable, relationship.toColumn)?.dataType);
    if (existing.isActive && targetsDate(existing)) {
        const remainingActiveDateRel = model.relationships.some((relationship) => relationship.id !== existing.id &&
...
tool('pbi_relationship_delete', 'Delete Relationship', 'Delete a relationship by id from the connected model.', {
...
tool('pbi_model_export', 'Export TMDL to Folder (folder-mode persistence)', 'Persist a folder-mode model to its .SemanticModel/definition TMDL on disk. Requires folderPath and refuses live mode; live Desktop persistence is the user pressing Ctrl+S in Desktop.', {
    folderPath: z
        .string()
        .describe('Required .SemanticModel/definition, .SemanticModel, .pbip, or containing folder to export to. Live Desktop models are refused; press Ctrl+S in Desktop for live persistence.'),
    model: MODEL_SELECT_FIELD,
}, { destructiveHint: false, idempotentHint: true }, async (input) => {
    const drv = getModelDriver();
    ...
    if (conn.mode !== 'folder') {
        throwInputValidationError('Model export refused: live models persist via Ctrl+S in Desktop.', {
...
tool('pbi_spec_validate', 'Validate DashboardSpec', 'Validate a DashboardSpec — the MODELING-PREP build contract handed from pbi-data-analyst to pbi-model-builder — against the v6 schema. The spec declares WHAT to build into the semantic model (measures, relationships, Date/calendar tables and their grain/coverage), NOT report layout, pages, or visuals. Returns { valid, errors[] }. The model-builder MUST call this and refuse to act when valid is false.', { spec: z.unknown().describe('The DashboardSpec object to validate.') }, { readOnlyHint: true, idempotentHint: true }, (input) => validateDashboardSpec(input.spec));
// -- Boot ------------------------------------------------------------------
...
export function buildModelingServer() {
    return buildServer();
}
async function main() {
