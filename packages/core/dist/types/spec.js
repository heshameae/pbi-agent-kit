...
export const MeasureSpecSchema = z
    ...
    .superRefine((v, ctx) => {
    const anySet = [v.bridgeFrom, v.bridgeTo, v.bridgeVia, v.bridgeCovers].some((f) => f !== undefined);
    const allSet = v.bridgeFrom !== undefined &&
    ...
    if (anySet && !allSet) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'bridge metadata is all-or-none: bridgeFrom, bridgeTo, bridgeVia, and a non-empty bridgeCovers must all be present',
        });
    }
...
const TIME_INTELLIGENCE_PATTERN = new RegExp(`\\b(?:${TIME_INTELLIGENCE_FUNCTIONS.join('|')})\\s*\\(`, 'i');
export function expressionUsesTimeIntelligence(expression) {
...
export const DashboardSpecSchema = z
    ...
    const confirmedMeasureIntents = new Map(v.measureIntents
    ...
    v.missingMeasures.forEach((measure, index) => {
        ...
        else if (expressionUsesTimeIntelligence(measure.expression) &&
...
function hasConfirmedTimeIntelligenceEvidence(intent) {
