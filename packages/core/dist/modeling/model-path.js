 ...
 */
export function resolveSemanticModelDefinition(input) {
 ...
 */
export function resolveSiblingSemanticModelDefinition(reportDefinitionPath) {
    const reportDefinition = path.resolve(reportDefinitionPath);
    const reportFolder = path.basename(reportDefinition) === 'definition'
...
function collectCandidates(start) {
    const candidates = [];
    if (!existsSync(start))
        return candidates;
...
function uniqueExistingDefinitionDirs(candidates) {
    const seen = new Set();
    const out = [];
    for (const candidate of candidates) {
        ...
        if (!isDirectory(resolved))
            continue;
        if (!looksLikeDefinitionFolder(resolved))
            continue;
        out.push(resolved);
    }
    return out;
}
function looksLikeDefinitionFolder(definitionPath) {
    return (existsSync(path.join(definitionPath, 'database.tmdl')) ||
...
//# sourceMappingURL=model-path.js.map