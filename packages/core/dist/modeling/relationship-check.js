...
import { directedFilterEdgesFromRelationships, edgeDisjointDirectedPaths, pathsDifferByIntermediate, } from './field-index.js';
// Unordered table-pair key (A|B === B|A), matching detectAmbiguousPaths.
function pairKey(a, b) {
    return a < b ? `${a}|${b}` : `${b}|${a}`;
}
export function relationshipCheck(candidate, model, options = {}) {
    const blocking = [];
    const warnings = [];
    ...
    if (candidate.isActive !== false) {
        ...
        const conflict = model.relationships.some((r) => r.isActive &&
...
function createsAmbiguousDiamond(model, candidate, ignoreRelationshipId) {
    const preEdges = directedFilterEdgesFromRelationships(model.relationships.filter((r) => r.id !== ignoreRelationshipId));
    ...
    for (const src of nodes) {
        for (const dst of nodes) {
            if (src === dst)
                continue;
            if (isDiamond(postEdges, src, dst) && !isDiamond(preEdges, src, dst))
                return true;
        }
    }
    return false;
...
export function typesCompatible(a, b) {
    ...
    if (leftDataType === rightDataType)
        return true;
    if (isNumericType(leftDataType) && isNumericType(rightDataType))
        return true;
    if (isTemporalType(leftDataType) && isTemporalType(rightDataType))
        return true;
    return false;
}
...
function detectAmbiguousPaths(model) {
...
function pushAdj(adj, from, to, relId) {
    const list = adj.get(from) ?? [];
    list.push({ neighbor: to, relId });
    adj.set(from, list);
}
function dedupeByRelId(items) {
    const out = [];
    const seen = new Set();
...
//# sourceMappingURL=relationship-check.js.map