...
export function parseTMDLFolder(definitionPath) {
    ...
    const relationships = existsSync(relationshipsFile)
...
export function parseTableFile(content) {
    ...
    let storageMode;
    let currentPartitionSourceKind;
    const columns = [];
    const measures = [];
    ...
    while (i < lines.length) {
        ...
        if (tableName !== null) {
            if (/^isHidden(?::\s*true)?$/i.test(line))
                tableIsHidden = true;
            if (/^calculated$/i.test(line))
                tableIsCalculated = true;
            ...
            if (descMatch?.[1] !== undefined)
                tableDescription = descMatch[1].trim();
            ...
            if (storageMode === undefined) {
                ...
                if (modeMatch?.[1])
                    storageMode = normalizeStorageMode(modeMatch[1]);
            }
        }
        ...
        if (parsedColumnHeader && tableName !== null) {
            ...
            const col = buildColumn(tableName, colName, inlineExpression, hasEquals, block.body, lines, i);
            columns.push(col);
            i = block.nextIndex;
            continue;
        }
...
export function parseRelationshipsFile(content) {
    const lines = content.split(/\r?\n/);
    const out = [];
    let i = 0;
    while (i < lines.length) {
    ...
    while (i < lines.length) {
        ...
        if (permHeader && roleName !== null) {
            ...
            if (inlineExpression !== undefined)
                exprLines.push(inlineExpression);
...
function collectBlock(lines, headerIndex) {
...
function collectSourceExpression(lines, sourceIndex, inline) {
...
function unquoteAnnotationValue(s) {
    const t = s.trim();
...
function splitHeaderOnEquals(rest) {
...
function parseMeasureHeader(rest) {
    return splitHeaderOnEquals(rest);
}
function parsePartitionSourceKind(line) {
...
function parseColumnHeader(rest) {
    return splitHeaderOnEquals(rest);
}
function buildColumn(table, name, inlineExpression, hasEquals, body, allLines, headerIndex) {
    ...
    for (const raw of body) {
        ...
        if (key && rawValue) {
            ...
            if (normalizedKey === 'datatype')
                dataType = value;
            else if (normalizedKey === 'summarizeby')
                summarizeBy = value;
            else if (normalizedKey === 'sourcecolumn')
                sourceColumn = unquoteIdent(value);
            else if (normalizedKey === 'datacategory')
                dataCategory = unquoteIdent(value);
            else if (normalizedKey === 'formatstring')
                formatString = value;
            else if (normalizedKey === 'ishidden')
                isHidden = parseTmdlBoolean(value);
            else if (normalizedKey === 'iskey')
                isKey = parseTmdlBoolean(value);
            else if (normalizedKey === 'description')
                description = value;
            else if (normalizedKey === 'displayfolder')
                displayFolder = value;
            else if (normalizedKey === 'sortbycolumn')
                sortByColumn = unquoteIdent(value);
            else if (normalizedKey === 'isavailableinmdx')
                isAvailableInMdx = parseTmdlBoolean(value);
            continue;
        }
    ...
    if (aboveDescription !== undefined)
        description = aboveDescription;
...
function buildMeasure(table, name, inlineExpression, body, allLines, headerIndex) {
    let formatString;
    let isHidden = false;
    let description;
    let displayFolder;
...
function buildRelationship(id, body) {
    let fromTable = null;
    let fromColumn = null;
    let toTable = null;
    let toColumn = null;
    let isActive = true;
    let crossFilteringBehavior = 'single';
    let fromCardinality;
    let toCardinality;
    let relyOnReferentialIntegrity;
    for (const raw of body) {
        const line = raw.trim();
        if (!line)
            continue;
...
function splitTableColumn(value) {
...
function findQuotedIdentifierEnd(value, start) {
    for (let i = start + 1; i < value.length; i++) {
        if (value[i] !== "'")
            continue;
        if (value[i + 1] === "'") {
...
//# sourceMappingURL=tmdl-parser.js.map