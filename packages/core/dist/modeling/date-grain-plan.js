 ...
 */
export function readGovernedDatePolicy(table) {
    ...
    const futureHorizonDays = Number.isFinite(parsedHorizon) && parsedHorizon >= 0
...
export function planDateGrain(model, options, evidence = []) {
    const blockers = [];
    const tableByName = new Map(model.tables.map((table) => [table.name, table]));
    const evidenceByKey = new Map(evidence.map((item) => [probeKey(item.tableName, item.dateColumn), item]));
    const requiredCoverageFacts = options.dateTable && options.dateColumn
        ...
        : options.facts;
    const dateCoverage = options.dateTable && options.dateColumn && options.dateTableCoverageEvidence
        ? planDateTableCoverage(model, {
            dateTable: options.dateTable,
            dateColumn: options.dateColumn,
    ...
    const facts = options.facts.map((fact) => {
        ...
        const dateTruncatingMeasureCandidates = findDateTruncatingMeasureCandidates(model, fact.tableName, fact.dateColumn);
        const writePlan = buildWritePlan(model, observedGrain, dayRelationshipSafe, relationship, 
        ...
        dateCoverage?.markReadiness.ready === true, dateCoverage?.dateTable.keyProvenFromData === true);
        return {
            ...
            relationship,
            measureGuidance: buildMeasureGuidance(observedGrain, exactDayProof, dateTruncatingMeasureCandidates, dayRelationshipSafe),
            writePlan,
        };
    });
    return {
...
export function buildDateGrainProbeQuery(model, facts) {
    const tableByName = new Map(model.tables.map((table) => [table.name, table]));
    const rows = [];
...
export function parseDateGrainProbeResult(payload) {
...
export function buildDateTableCoverageProbeQuery(model, options) {
    const tableByName = new Map(model.tables.map((table) => [table.name, table]));
...
export function parseDateTableCoverageProbeResult(payload) {
...
export function planDateTableCoverage(model, options, evidence = { facts: [] }) {
    ...
    const dateTableEvidence = rawDateTableEvidence?.tableName === options.dateTable &&
        rawDateTableEvidence.dateColumn === options.dateColumn
        ? rawDateTableEvidence
        : undefined;
    if (rawDateTableEvidence &&
    ...
    if (table?.isAutoDateTable) {
        blockers.push({
            ...
            table: table.name,
            message: 'Auto date tables are generated implementation details, not governed date dimensions.',
        });
    }
    if (table && column && !isMarkedDateTable(table, column)) {
        ...
        if (keyProvenFromData) {
            warnings.push({
                ...
                table: table.name,
                column: column.name,
                message: 'The date key is proven clean from live data, but the dataCategory="Time" mark is not observable through the modeling read layer (a known Import-mode no-op, even after marking in Desktop). Relationships and explicit-column time intelligence (e.g. TOTALYTD over the date column) work from the proven key. Built-in/implicit time intelligence and the Date hierarchy UX still require marking the table in Power BI Desktop.',
            });
        }
        else {
            blockers.push({
                ...
                table: table.name,
                column: column.name,
                message: 'The governed date table must be marked as a Date table before it is used as the time-intelligence axis.',
            });
        }
    }
    if (column && !isTemporalColumn(column)) {
    ...
    const hasSourceProvenance = table?.expression !== undefined || (table?.partitionSources?.length ?? 0) > 0;
    if (model.modelPath === '(live)' &&
        table &&
        column &&
    ...
    if (!hasCompleteDateTableKeyProof(dateTableEvidence)) {
        blockers.push({
            ...
            table: options.dateTable,
            column: options.dateColumn,
            message: 'Date-table coverage is not proven by a live data probe. Do not edit calendar bounds or rely on this table for relationship rewrites from prompt judgment alone.',
        });
    }
    ...
    const factCoverage = requiredFacts.map((fact) => buildFactCoverage(model, fact, evidence.facts, dateTableEvidence));
    for (const coverage of factCoverage)
        blockers.push(...coverage.blocking);
    const dateMax = dateOnlyOrdinal(dateTableEvidence?.maxDate);
    const factMaxes = requiredFacts
        .map((fact) => evidence.facts.find((candidate) => candidate.tableName === fact.tableName && candidate.dateColumn === fact.dateColumn))
    ...
    const effectiveFutureHorizonDays = options.futureHorizonDays ?? persistedPolicy.futureHorizonDays ?? 0;
    const effectiveAllowEndAfterFactMax = options.allowCalendarEndAfterFactMax ?? persistedPolicy.allowCalendarEndAfterFactMax ?? false;
    if (hasCompleteDateTableKeyProof(dateTableEvidence) &&
        ...
        blockers.push({
            ...
            table: options.dateTable,
            column: options.dateColumn,
            message: 'The date table extends beyond the observed maximum date across the supplied facts without a sufficient explicit futureHorizonDays policy.',
        });
    }
    const recommendedRange = buildRecommendedDateRange(requiredFacts, evidence.facts);
    const onlyProofMissing = blockers.length > 0 &&
        blockers.every((blocker) => ['date-table-proof-missing', 'fact-date-proof-missing'].includes(blocker.code));
    ...
    return {
        ...
        markReadiness,
        autoDateTables: autoDateTableSummary(model),
        recommendation: 'Use one explicit, marked date table whose daily key covers the observed min/max dates of every related fact. Anchor default calendar bounds to observed fact min/max dates, not TODAY()/NOW(); future padding requires an explicit forecast horizon policy.',
    };
}
export function deriveRequiredDateCoverageFacts(model, options) {
    const facts = new Map();
    const setFact = (fact) => {
        facts.set(probeKey(fact.tableName, fact.dateColumn), fact);
    };
    const addDerivedFact = (fact) => {
        const table = model.tables.find((candidate) => candidate.name === fact.tableName);
        const column = table?.columns.find((candidate) => candidate.name === fact.dateColumn);
        if (!table || !column || !isTemporalColumn(column))
            return;
        if (table.name === options.dateTable || table.isAutoDateTable || isDateTableLike(table))
            return;
        setFact(fact);
    };
    for (const relationship of model.relationships) {
        const fromIsDate = relationship.fromTable === options.dateTable &&
            relationship.fromColumn === options.dateColumn;
        const toIsDate = relationship.toTable === options.dateTable && relationship.toColumn === options.dateColumn;
        if (fromIsDate) {
    ...
    for (const table of model.tables) {
        if (!looksLikeDateCoverageFactTable(model, table, options.dateTable))
            continue;
...
export function classifyObservedDateGrain(evidence) {
    if (!evidence)
        return 'unknown';
    if (!hasCompleteDateGrainProof(evidence))
        return 'unknown';
    const nonBlankDateCount = evidence.nonBlankDateCount;
    const distinctDateCount = evidence.distinctDateCount;
    if (nonBlankDateCount === 0 || distinctDateCount === 0)
        return 'empty';
...
function isExactDayGrainProof(evidence) {
    if (!evidence || !hasCompleteDateGrainProof(evidence))
        return false;
    const minDate = dateOnlyOrdinal(evidence.minDate);
    const maxDate = dateOnlyOrdinal(evidence.maxDate);
    if (minDate === undefined || maxDate === undefined || maxDate < minDate)
        return false;
    const observedDaySpan = maxDate - minDate + 1;
    return (evidence.nonBlankDateCount > 0 &&
...
function isDayRelationshipDateGrainSafe(observedGrain, evidence) {
    return (observedGrain === 'day' &&
...
function missingColumnPlan(fact) {
    return {
        tableName: fact.tableName,
        dateColumn: fact.dateColumn,
        metadata: {
...
function dateColumnMetadata(column) {
    if (!isTemporalColumn(column)) {
        return {
...
function resolveDateRelationship(model, factTable, factColumn, options) {
    const candidates = model.relationships
        .filter((relationship) => relationshipTouchesFactDate(relationship, factTable.name, factColumn.name))
        .filter((relationship) => relationshipMatchesRequestedDateDimension(relationship, factTable.name, factColumn.name, options))
    ...
    if (candidates.length > 1) {
        ...
        if (nonAuto.length === 1 && onlyNonAuto)
            return activeStatus(onlyNonAuto);
        return { status: 'ambiguous', candidates };
    }
    const onlyCandidate = candidates[0];
    if (candidates.length === 1 && onlyCandidate)
        return activeStatus(onlyCandidate);
...
function relationshipTouchesFactDate(relationship, tableName, columnName) {
    return ((relationship.fromTable === tableName && relationship.fromColumn === columnName) ||
        (relationship.toTable === tableName && relationship.toColumn === columnName));
}
function relationshipMatchesRequestedDateDimension(relationship, factTable, factColumn, options) {
    if (!options.dateTable && !options.dateColumn)
        return true;
    const dateEndpoint = dateEndpointForFactRelationship(relationship, factTable, factColumn);
    if (!dateEndpoint)
        return false;
    return ((!options.dateTable || dateEndpoint.table === options.dateTable) &&
        (!options.dateColumn || dateEndpoint.column === options.dateColumn));
}
function dateEndpointForFactRelationship(relationship, factTable, factColumn) {
...
function toExistingRelationshipPlan(relationship, model) {
    const activationCheck = relationship.isActive
        ...
        }, model, { ignoreRelationshipId: relationship.id });
...
function relationshipActivationShapeBlocking(relationship) {
    const blocking = [];
...
function activeStatus(plan) {
    return plan;
}
function otherRelationshipTable(plan, tableName) {
    return plan.fromTable === tableName ? plan.toTable : plan.fromTable;
}
function buildWritePlan(model, observedGrain, dayRelationshipSafe, relationship, coverageWritable, dateKeyProvenFromData) {
        ...
        return [
            {
                ...
                id: relationship.id,
                description: 'Observed day-level date values support activating the existing date relationship.',
            },
        ];
    }
    if (relationship.status === 'inactive' && !relationship.canActivate) {
    ...
    if (relationship.status === 'missing' &&
        ...
        return [
            {
                ...
                crossFilteringBehavior: 'single',
                isActive: true,
                description: 'Observed day-level date values support creating an active many-to-one date relationship.',
            },
        ];
    }
    return [];
}
function buildMeasureGuidance(observedGrain, exactDayProof, candidates, dayRelationshipSafe = false) {
    if (observedGrain === 'day' && dayRelationshipSafe) {
        return {
            plainSumSafe: true,
    ...
    if (observedGrain === 'day') {
        return {
            ...
            dateTruncatingMeasureCandidates: candidates,
            message: 'Observed fact dates look date-level but do not provide complete date-only proof. Do not rewrite target/actual measures, activate date relationships, or use day-level visuals automatically until grain is proven by data.',
        };
    }
    if (observedGrain === 'month-start' || observedGrain === 'month-single-date') {
        return {
            ...
            dateTruncatingMeasureCandidates: candidates,
            message: 'Observed fact dates have one distinct date value per month. Do not repeat the monthly value across every day; use a month-grain axis/key or explicit daily allocation logic.',
        };
    }
    if (observedGrain === 'submonthly') {
        return {
            ...
            safeVisualDateGrain: 'unknown',
            dateTruncatingMeasureCandidates: candidates,
            message: 'Observed fact dates have more than one date in at least one month, but not enough repeated evidence to prove daily grain. Treat this as a grain-review case before rewriting target/actual measures.',
        };
    }
    return {
        plainSumSafe: false,
        ...
        safeVisualDateGrain: 'unknown',
        dateTruncatingMeasureCandidates: candidates,
        message: 'Date grain is not proven. Run the live date-grain probe before changing date relationships or grain-sensitive target measures.',
    };
}
function relationshipTargetsGovernedDateEndpoint(model, relationship, dateKeyProvenFromData) {
    if (relationship.status !== 'inactive' && relationship.status !== 'missing')
        return true;
    ...
    if (table === undefined || column === undefined || table.isAutoDateTable)
        return false;
...
function findDateTruncatingMeasureCandidates(model, factTable, dateColumn) {
    const candidates = [];
    for (const table of model.tables) {
...
function isDateTruncatingMeasure(measure, factTable, dateColumn) {
    const compact = measure.expression.replace(/\s+/g, '').toUpperCase();
    if (!compact.includes('TREATAS('))
        return false;
    const hasDateTruncation = compact.includes('DATE(YEAR(') ||
        compact.includes('EOMONTH(') ||
        compact.includes('STARTOFMONTH(');
    if (!hasDateTruncation)
        return false;
    const expressionUpper = measure.expression.toUpperCase();
    return exactColumnRefs(factTable, dateColumn).some((ref) => expressionUpper.includes(ref.toUpperCase()));
}
function exactColumnRefs(tableName, columnName) {
...
function autoDateTableSummary(model) {
    ...
    return {
        ...
        ...(names.length > 0
            ? {
                recommendation: 'Disable Auto Date/Time and use the governed date table; auto date tables add repeated calendar structures and slow model inspection.',
            }
            : {}),
    };
}
function buildProbeRow(tableName, dateColumn, kind = 'fact') {
...
function toDateTableEvidence(row) {
    const tableName = stringValue(row, '__table', 'tableName', 'table') ?? '';
    const dateColumn = stringValue(row, '__column', 'dateColumn', 'column') ?? '';
    const rowCount = numberValue(row, 'rowCount');
    const nonBlankDateCount = numberValue(row, 'nonBlankDateCount');
...
function buildFactCoverage(model, fact, factsEvidence, dateTableEvidence) {
    const evidence = factsEvidence.find((candidate) => candidate.tableName === fact.tableName && candidate.dateColumn === fact.dateColumn);
    const blocking = [];
...
function buildRecommendedDateRange(facts, factsEvidence) {
    const evidenceByKey = new Map(factsEvidence.map((evidence) => [probeKey(evidence.tableName, evidence.dateColumn), evidence]));
    const requestedEvidence = facts.map((fact) => evidenceByKey.get(probeKey(fact.tableName, fact.dateColumn)));
    if (requestedEvidence.length === 0 ||
        requestedEvidence.some((evidence) => !hasCompleteDateGrainProof(evidence) ||
            dateOnlyOrdinal(evidence.minDate) === undefined ||
            dateOnlyOrdinal(evidence.maxDate) === undefined)) {
        return {
            requiresExplicitForecastHorizon: true,
            message: 'Observed fact min/max dates are not fully proven. Do not invent calendar bounds or use TODAY()/NOW(); run the live proof with every relevant fact date column.',
        };
    }
    ...
    return {
        ...
        calendarEndDate,
        requiresExplicitForecastHorizon: true,
        message: 'Default calendar bounds should be anchored to observed fact min/max dates. Extend beyond the observed max only when the user supplies an explicit forecast horizon policy.',
    };
}
function toIsoDate(dayOrdinal) {
...
function dateOnlyOrdinal(value) {
    ...
    if (typeof value === 'string') {
        ...
        if (/^-?\d+$/.test(trimmed))
            return dateSerialOrdinal(Number(trimmed));
        return undefined;
    }
    if (typeof value === 'number')
        return dateSerialOrdinal(value);
    return undefined;
}
...
function dateSerialOrdinal(serial) {
...
function isMarkedDateTable(table, column) {
...
export function findCalendarSourceRisks(sources) {
    const risks = [];
    for (const source of sources) {
        const expression = source.expression;
        if (!expression)
            continue;
        const hasVolatileAnchor = /\b(?:TODAY|NOW|UTCNOW)\s*\(/i.test(expression) ||
            /\bDateTime(?:Zone)?\.(?:LocalNow|UtcNow|FixedLocalNow|FixedUtcNow)\s*\(/i.test(expression) ||
            /\bDate\.(?:From|StartOfDay)\s*\(\s*DateTime(?:Zone)?\.(?:LocalNow|UtcNow|FixedLocalNow|FixedUtcNow)\s*\(/i.test(expression);
        if (hasVolatileAnchor) {
            risks.push({
                code: 'volatile-calendar-anchor',
                ...(source.kind !== undefined ? { sourceKind: source.kind } : {}),
                message: 'The date table definition uses a volatile current-date function as a calendar bound. Use observed fact min/max dates and an explicit forecast horizon policy instead of the system date.',
            });
        }
        const calendarLike = /\b(?:CALENDAR|CALENDARAUTO)\s*\(/i.test(expression) ||
        ...
        const hasLiteralBounds = /\bDATE\s*\(\s*\d{4}\s*,\s*\d{1,2}\s*,\s*\d{1,2}\s*\)/i.test(expression) ||
        ...
        if (hasLiteralBounds && (calendarLike || source.kind !== undefined)) {
            risks.push({
                ...
                ...(source.kind !== undefined ? { sourceKind: source.kind } : {}),
                message: 'The date table definition uses literal calendar bounds. Prove observed fact min/max dates with pbi_model_plan_date_table and only extend with an explicit futureHorizonDays policy.',
            });
        }
    }
    return risks;
}
function findDateTableSourceRisks(table) {
    if (!table)
        return [];
    const sources = [
...
function looksLikeDateCoverageFactTable(model, table, dateTableName) {
 ...
 */
export function isDataProvenDailyKey(evidence) {
    return (hasCompleteDateTableKeyProof(evidence) &&
        evidence.blankDateCount === 0 &&
...
export function hasCompleteDateGrainProof(evidence) {
    return (evidence !== undefined &&
        evidence.rowCount !== undefined &&
        evidence.nonBlankDateCount !== undefined &&
        evidence.distinctDateCount !== undefined &&
...
function zipColumnarRows(record) {
...
function stringValue(row, ...aliases) {
    const value = findValue(row, ...aliases);
    if (typeof value === 'string')
        return value;
...
function findValue(row, ...aliases) {
    ...
    for (const [key, value] of Object.entries(row)) {
        if (normalizedAliases.has(normalizeResultKey(key)))
            return value;
    }
    return undefined;
}
...
//# sourceMappingURL=date-grain-plan.js.map