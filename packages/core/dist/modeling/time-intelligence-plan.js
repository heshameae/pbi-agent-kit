...
export function parseTimeIntelligencePeriod(period) {
    if (period === undefined)
        return undefined;
    const p = period.trim().toUpperCase();
    if (p === 'YTD' || p.includes('YEAR TO DATE') || p.includes('YEAR-TO-DATE'))
        return 'YTD';
    if (p === 'QTD' || p.includes('QUARTER TO DATE') || p.includes('QUARTER-TO-DATE'))
        return 'QTD';
    if (p === 'MTD' || p.includes('MONTH TO DATE') || p.includes('MONTH-TO-DATE'))
        return 'MTD';
    return undefined;
}
...
export function isYearEndDateLiteral(arg) {
...
export function calendarOvershootsFactDay(calendarMaxDate, factMaxDate) {
    const calendar = dayOrdinal(calendarMaxDate);
    const fact = dayOrdinal(factMaxDate);
    if (calendar === undefined || fact === undefined)
        return false;
...
function columnRef(table, column) {
...
function totalCall(fn, baseExpression, datesRef, opts = {}) {
    ...
    if (opts.yearEndDate !== undefined && fn === 'TOTALYTD')
        args.push(opts.yearEndDate);
    return `${fn}(${args.join(', ')})`;
}
...
export function buildTimeIntelligenceMeasureExpression(input) {
...
export function detectCalendarMaxAnchorCap(expression, dateTable, dateColumn) {
    ...
    if (new RegExp(`calculate\\((?:max|lastdate)\\(${ax}\\),(?:removefilters|allexcept|allselected|all)\\(`).test(collapsed)) {
        return true;
    }
    ...
    return new RegExp(`maxx\\((?:values|allselected|all)\\((?:${ax}|${tb})\\),${ax}\\)`).test(collapsed);
}
//# sourceMappingURL=time-intelligence-plan.js.map