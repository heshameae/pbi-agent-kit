...
export const BPA_RULES: ReadonlyArray<BPARule> = [
  {
    ...
    category: 'DAX',
    check: (model) =>
      forEachExpressionObject(model, (o) => {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      for (const r of model.relationships) {
        ...
        const siblingActive = model.relationships.find(
          (a) => a.id !== r.id && a.isActive && samePair(a, r) && columnPairSignature(a) !== rSig,
        );
        if (siblingActive) {
          findings.push(
            violation('MOD002', 'error', 'Modeling', `Relationship.${r.id}`, {
          ...
          findings.push(
            violation('MOD002', 'warning', 'Modeling', `Relationship.${r.id}`, {
      ...
      model.relationships
        ...
        .map((r) => {
          ...
          if (r.crossFilteringBehavior === 'both') {
            ...
            const severity: Severity = bridged ? 'warning' : 'error';
    ...
    check: (model) => {
      ...
      for (const t of model.tables) {
        for (const col of t.columns) {
          ...
          if (!col.expression || col.expression.trim() === '') continue;
          const result = daxReferenceCheck(col.expression, model, { hostTable: t.name });
          for (const ref of result.missing) {
            findings.push(
              violation('DAX005', 'warning', 'DAX', columnRefRaw(col.table, col.name), {
  ...
  // === Tier 1: relationships / star-schema (FINAL contract) ===============
  {
    ...
    check: (model) => {
      ...
      const findings: BPAViolation[] = [];
      for (const t of model.tables) {
        if (t.isAutoDateTable) continue; // MOD001 owns auto-date tables.
        if (related.has(t.name)) continue;
        ...
        if (isDeliberatelyDisconnected(t)) {
          findings.push(
            violation('MOD008', 'info', 'Modeling', `Table.${t.name}`, {
          ...
          findings.push(
            violation('MOD008', 'error', 'Modeling', `Table.${t.name}`, {
          ...
          findings.push(
            violation('MOD008', 'warning', 'Modeling', `Table.${t.name}`, {
    ...
    check: (model) => {
      ...
      for (const r of model.relationships) {
        ...
        if (
          ...
          findings.push(
            violation('MOD009', 'error', 'Modeling', `Relationship.${r.id}`, {
    ...
    check: (model) => {
      ...
      for (let a = 0; a < facts.length; a++) {
        for (let b = a + 1; b < facts.length; b++) {
          ...
          const fb = facts[b];
          if (!fa || !fb) continue;
          if (hasUndirectedRelationshipPath(index, fa.name, fb.name)) continue;
          ...
          for (const colName of shared) {
            ...
            reported.add(key);
            findings.push(
              violation('MOD010', 'warning', 'Modeling', `Table.${fa.name}`, {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      for (const r of model.relationships) {
        ...
        findings.push(
          violation('MOD011', severity, 'Modeling', `Relationship.${r.id}`, {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      const findings: BPAViolation[] = [];
      for (const t of model.tables) {
        if (!fromSides.has(t.name) || !toSides.has(t.name)) continue;
        if (classifyTable(model, t.name).kind === 'fact') continue;
        findings.push(
          violation('MOD012', 'warning', 'Modeling', `Table.${t.name}`, {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      if (total === 0) return [];
      const risky = model.relationships.filter(
        (r) => r.crossFilteringBehavior === 'both' || r.cardinality === 'manyToMany',
      ).length;
    ...
    check: (model) => {
      ...
      for (const r of model.relationships) {
        ...
        for (const [tableName, col] of [
          ...
          seen.add(key);
          findings.push(
            violation('MOD015', 'info', 'Modeling', columnRefRaw(tableName, col.name), {
      ...
      for (const bridge of Object.values(index.treatasBridgeMeasures)) {
        ...
        findings.push(
          violation(
            ...
            'info',
            'Modeling',
            `'${bridge.measure.table}'[${bridge.measure.name}]`,
            {
    ...
    check: (model) => {
      ...
      for (const t of model.tables) {
        for (const c of t.columns) {
          ...
          if (looksLikeKeyName(c.name) && (!c.summarizeBy || c.summarizeBy === 'none')) continue;
          if (!c.formatString || c.formatString.trim() === '') {
            findings.push(
              violation('FMT003', 'warning', 'Formatting', columnRef(c), {
  ...
  },
  {
    ...
    check: (model) => {
      const findings: BPAViolation[] = [];
      const check = (objectRef: string, name: string): void => {
        if (hasLeadingTrailingSpace(name) || hasControlChars(name)) {
          findings.push(
            violation('NAM002', 'error', 'Naming', objectRef, {
        ...
        if (isReservedWord(name)) {
          findings.push(
            violation('NAM002', 'warning', 'Naming', objectRef, {
      ...
      for (const t of model.tables) {
        ...
        for (const c of t.columns) check(columnRef(c), c.name);
        for (const m of t.measures) check(measureRef(m), m.name);
      }
      return findings;
    },
  },
  {
    ...
    check: (model) => {
      const modelExpressesSources = model.tables.some((t) =>
        t.columns.some((c) => !c.isCalculated && c.sourceColumn && c.sourceColumn.trim() !== ''),
      );
      if (!modelExpressesSources) return [];
      const findings: BPAViolation[] = [];
      for (const t of model.tables) {
        if (t.isCalculated || t.isAutoDateTable) continue;
        for (const c of t.columns) {
          if (c.isCalculated) continue;
          if (!c.sourceColumn || c.sourceColumn.trim() === '') {
            findings.push(
              violation('E1', 'error', 'ErrorPrevention', columnRef(c), {
      ...
      for (const t of model.tables) {
        for (const m of t.measures) {
          ...
          for (const match of withoutQualified.matchAll(/\[([^\]]+)\]/g)) {
            const name = match[1];
            if (!name) continue;
            if (measureNames.has(name)) continue; // a bare measure ref — DAX007 territory, allowed here.
            if (columnNames.has(name)) {
              findings.push(
                violation('DAX006', 'error', 'DAX', measureRef(m), {
        ...
        for (const m of t.measures) {
          ...
          for (const match of expr.matchAll(/('([^']+)'|([A-Za-z_][\w .-]*))\[([^\]]+)\]/g)) {
            ...
            if (measureNames.has(fieldName) && !columnRefs.has(`${tableName}[${fieldName}]`)) {
              findings.push(
                violation('DAX007', 'error', 'DAX', measureRef(m), {
  ...
  },
  {
    ...
    category: 'DAX',
    check: (model) => {
      const byNormalized = new Map<string, { table: string; name: string }[]>();
      for (const t of model.tables) {
        for (const m of t.measures) {
      ...
      forEachMeasure(model, (m) => {
        ...
        if (/\bINTERSECT\s*\(/i.test(expr)) {
          return violation('DAX009', 'warning', 'DAX', measureRef(m), {
            message:
              'Uses INTERSECT; TREATAS is usually faster and clearer for applying a virtual relationship.',
  ...
  },
  {
    ...
    check: (model) =>
      forEachMeasure(model, (m) => {
        const expr = stripDaxComments(m.expression);
        if (!/\bCALCULATE\s*\(/i.test(expr)) return null;
        ...
        if (/\bFILTER\s*\(\s*('[^']+'|[A-Za-z_][\w]*|ALL\s*\([^)]*\))\s*,/i.test(expr)) {
          return violation('DAX011', 'warning', 'DAX', measureRef(m), {
            message:
              'CALCULATE wraps FILTER over an entire table; this materializes the full table. Filter on a single column instead.',
    ...
    check: (model) => {
      ...
      for (let i = 0; i < tables.length; i++) {
        for (let j = 0; j < tables.length; j++) {
          ...
          if (!pathsDifferByIntermediate(paths)) continue;
          reported.add(key);
          findings.push(
            violation('MOD017', 'error', 'Modeling', `Model.AmbiguousPath(${key})`, {
    ...
    check: (model) => {
      ...
      for (const d of model.tables) {
        ...
        interface FactGrain {
          ...
          readonly grain: 'day' | 'coarse' | 'unknown';
        }
        const factGrains: FactGrain[] = [];
        for (const r of model.relationships) {
          if (!r.isActive) continue;
          let factName: string | undefined;
          ...
          if (factName === undefined || toCol === undefined) continue;
          const cls = classifyTable(model, factName);
          if (cls.kind !== 'fact' || cls.confidence < 0.85) continue;
        ...
        for (const a of factGrains) {
          for (const b of factGrains) {
            if (a.fact === b.fact) continue;
            if (a.toColumn === b.toColumn) continue; // same key → never fire
            if (a.grain !== 'day' || b.grain !== 'coarse') continue;
            const key = `${[a.fact, b.fact].sort().join('|')}|${d.name}`;
            if (reported.has(key)) continue;
            reported.add(key);
            findings.push(
              violation('MOD019', 'warning', 'Modeling', `Table.${b.fact}`, {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      const seen = new Set<string>();
      for (const r of model.relationships) {
        if (r.cardinality === 'manyToMany') continue;
        ...
        seen.add(key);
        findings.push(
          violation('MOD020', 'info', 'Modeling', columnRefRaw(r.toTable, r.toColumn), {
  ...
  },
  {
    ...
    check: (model) =>
      model.relationships
        ...
        .map((r) =>
          violation('MOD021', 'info', 'Modeling', `Relationship.${r.id}`, {
    ...
    check: (model) => {
      ...
      for (const t of model.tables) {
        ...
        for (const c of t.columns) {
          ...
          if (!isNumericType(c.dataType)) continue;
          if (!c.summarizeBy || c.summarizeBy.toLowerCase() === 'none') continue;
          if (looksLikeKeyName(c.name)) continue; // MOD014 owns key-named columns.
          offenders.push(c.name);
        }
        if (offenders.length === 0) continue;
        const list = offenders.map((n) => `"${n}"`).join(', ');
        findings.push(
          violation('MOD022', 'info', 'Modeling', `Table.${t.name}`, {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      if (securedTables.size === 0) return [];
      const findings: BPAViolation[] = [];
      for (const t of model.tables) {
        for (const m of t.measures) {
          const expr = stripDaxComments(m.expression);
          ...
          for (const call of expr.matchAll(/\bUSERELATIONSHIP\s*\(([^)]*)\)/gi)) {
            ...
            for (const ref of args.matchAll(QUALIFIED_REF_RE)) {
              ...
              if (tableName && securedTables.has(tableName)) hitTables.add(tableName);
            }
          }
          for (const tableName of hitTables) {
            findings.push(
              violation('MOD023', 'error', 'Modeling', measureRef(m), {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      if (dynamicSecured.size === 0) return [];
      const findings: BPAViolation[] = [];
      for (const r of model.relationships) {
        if (r.cardinality !== 'manyToMany') continue;
        const securedEndpoint = dynamicSecured.has(r.fromTable)
          ...
          : dynamicSecured.has(r.toTable)
            ? r.toTable
            : undefined;
        if (securedEndpoint === undefined) continue;
        findings.push(
          violation('MOD024', 'warning', 'Modeling', `Relationship.${r.id}`, {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      const findings: BPAViolation[] = [];
      for (const r of model.relationships) {
        if (r.crossFilteringBehavior !== 'both') continue;
        ...
        if (securedEndpoint === undefined) continue;
        findings.push(
          violation('MOD025', 'warning', 'Modeling', `Relationship.${r.id}`, {
  ...
  },
  {
    ...
    category: 'Maintenance',
    check: (model) => {
      if (!modelExpressesDescriptions(model)) return [];
      const findings: BPAViolation[] = [];
      ...
      for (const t of model.tables) {
        ...
        if (!t.isHidden && blank(t.description)) {
          findings.push(
            violation('MOD026', 'info', 'Maintenance', `Table.${t.name}`, {
        ...
        for (const col of t.columns) {
          if (!col.isHidden && blank(col.description)) {
            findings.push(
              violation('MOD026', 'info', 'Maintenance', columnRef(col), {
        ...
        for (const m of t.measures) {
          if (!m.isHidden && blank(m.description)) {
            findings.push(
              violation('MOD026', 'info', 'Maintenance', measureRef(m), {
  ...
  },
  {
    ...
    category: 'Maintenance',
    check: (model) => {
      if (!modelExpressesMeasureDisplayFolders(model)) return [];
      const findings: BPAViolation[] = [];
      for (const t of model.tables) {
        ...
        if (unfoldered.length > 10) {
          findings.push(
            violation('MOD027', 'info', 'Maintenance', `Table.${t.name}`, {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      const findings: BPAViolation[] = [];
      for (const r of model.relationships) {
        if (r.relyOnReferentialIntegrity !== true) continue;
        const fromMode = byTable.get(r.fromTable)?.storageMode;
        const toMode = byTable.get(r.toTable)?.storageMode;
        if (fromMode !== 'directQuery' && toMode !== 'directQuery') continue;
        findings.push(
          violation('MOD028', 'warning', 'Modeling', `Relationship.${r.id}`, {
      ...
      forEachMeasure(model, (m) => {
        ...
        if (/\bEVALUATEANDLOG\s*\(/i.test(expr)) {
          return violation('DAX012', 'warning', 'DAX', measureRef(m), {
            message:
              'Measure uses EVALUATEANDLOG, a debugging function; remove it before shipping to production.',
      ...
      forEachMeasure(model, (m) => {
        ...
        if (sumForm.test(expr) || divideForm.test(expr)) {
          return violation('DAX013', 'warning', 'DAX', measureRef(m), {
            message:
              'Measure mixes a literal number with a raw division (e.g. 1-(x/y) or 1-DIVIDE(...)); this is brittle when the denominator is blank/zero. Wrap the ratio in DIVIDE and handle the blank case explicitly.',
      ...
      forEachMeasure(model, (m) => {
        ...
        if (p1.test(expr) || p2.test(expr) || p3.test(expr)) {
          return violation('DAX014', 'warning', 'DAX', measureRef(m), {
            message:
              'Measure suppresses BLANK (returns 0 via +0 / COALESCE(...,0) / IF(ISBLANK,0,...)); if not intentional, this inflates SUMMARIZECOLUMNS result sets with spurious all-zero rows.',
  ...
  },
  {
    ...
    check: (model) =>
      forEachMeasure(model, (m) => {
        if (m.isHidden) return null;
        if (!m.formatString || m.formatString.trim() === '') return null; // FMT001 owns blank.
        if (!looksLikePercentageMeasure(m.name)) return null;
        if (m.formatString.includes('%')) return null;
      ...
      for (const t of model.tables) {
        for (const c of t.columns) {
          ...
          if (isGeoString || isLatLong) {
            findings.push(
              violation('FMT006', 'info', 'Formatting', columnRef(c), {
    ...
    check: (model) => {
      ...
      for (const t of model.tables) {
        ...
        for (const c of t.columns) {
          ...
          findings.push(
            violation('FMT007', 'warning', 'Formatting', columnRef(c), {
  ...
  },
  {
    ...
    category: 'ErrorPrevention',
    check: (model) => {
      const modelExpressesCalcColumns = model.tables.some((t) =>
        t.columns.some((c) => c.isCalculated && c.expression && c.expression.trim() !== ''),
      );
      if (!modelExpressesCalcColumns) return [];
      const findings: BPAViolation[] = [];
      for (const t of model.tables) {
        for (const c of t.columns) {
          if (!c.isCalculated) continue;
          if (!c.expression || c.expression.trim() === '') {
            findings.push(
              violation('E3', 'error', 'ErrorPrevention', columnRef(c), {
  ...
  },
  {
    ...
    check: (model) => {
      ...
      for (const t of model.tables) {
        for (const c of t.columns) {
          if (c.isAvailableInMdx !== false) continue; // only explicit false.
          const isTarget = targets.has(`${t.name}[${c.name}]`);
          const hasOwnSort = !!c.sortByColumn && c.sortByColumn.trim() !== '';
          if (!isTarget && !hasOwnSort) continue;
          findings.push(
            violation('E4', 'error', 'ErrorPrevention', columnRef(c), {
...
// A DAX-expression-bearing object: every measure AND every calculated column
// with a non-empty expression. Lets expression-hygiene rules (DAX001 + the
// reference check) run over calc columns without duplicating their matchers —
// the iteration source widens, the regex bodies stay identical.
interface ExprObject {
  readonly table: string;
  readonly name: string;
  readonly expression: string;
  readonly isHidden: boolean;
  readonly kind: 'measure' | 'calcColumn';
  readonly object: string; // pre-rendered 'Table'[Name] ref
}

function forEachExpressionObject(
  model: TMDLModel,
  fn: (o: ExprObject) => BPAViolation | null,
): ReadonlyArray<BPAViolation> {
  const out: BPAViolation[] = [];
  for (const t of model.tables) {
    for (const m of t.measures) {
    ...
    for (const c of t.columns) {
      if (!c.isCalculated) continue;
      if (!c.expression || c.expression.trim() === '') continue;
      const v = fn({
        table: c.table,
        name: c.name,
...
function relationshipEndpointTables(model: TMDLModel): Set<string> {
...
function isDeliberatelyDisconnected(table: TMDLTable): boolean {
  if (table.isAutoDateTable) return true;
  if (table.columns.length <= 1 && table.measures.length === 0) return true;
  if (table.isCalculated && table.columns.length <= 2 && table.measures.length === 0) return true;
  return false;
}
...
function looksIntrinsicallyFactLike(table: TMDLTable): boolean {
  if (table.measures.length > 0) return true;
  return table.columns.some(
    (c) => isNumericType(c.dataType) && !!c.summarizeBy && c.summarizeBy.toLowerCase() !== 'none',
  );
}
...
function sharedCategoricalColumnNames(a: TMDLTable, b: TMDLTable): string[] {
...
function isReservedWord(name: string): boolean {
  return /^(DATE|TIME|YEAR|MONTH|DAY|HOUR|MINUTE|SECOND|NOW|TODAY|TRUE|FALSE|BLANK)$/i.test(
    name.trim(),
  );
}
...
function looksLikePercentageMeasure(name: string): boolean {
...
function isBridgeTable(model: TMDLModel, tableName: string): boolean {
  const table = model.tables.find((t) => t.name === tableName);
  if (!table) return false;
  if (table.measures.length > 0) return false;
  const toSideCount = model.relationships.filter((r) => r.toTable === tableName).length;
  if (toSideCount < 2) return false;
  return table.columns.every(
    (c) =>
      c.isKey ||
...
function samePair(a: TMDLRelationship, b: TMDLRelationship): boolean {
  const ka =
    a.fromTable < a.toTable ? `${a.fromTable}|${a.toTable}` : `${a.toTable}|${a.fromTable}`;
  const kb =
    b.fromTable < b.toTable ? `${b.fromTable}|${b.toTable}` : `${b.toTable}|${b.fromTable}`;
  return ka === kb;
}
function columnPairSignature(r: TMDLRelationship): string {
...
function securedTableSet(model: TMDLModel): Set<string> {
  const set = new Set<string>();
  for (const role of model.roles ?? []) {
    for (const perm of role.tablePermissions) {
      if (perm.filterExpression.trim() !== '') set.add(perm.table);
    }
  }
  return set;
}
function dynamicSecuredTableSet(model: TMDLModel): Set<string> {
  const set = new Set<string>();
  for (const role of model.roles ?? []) {
    for (const perm of role.tablePermissions) {
      if (
        perm.filterExpression.trim() !== '' &&
...
function modelExpressesDescriptions(model: TMDLModel): boolean {
...
function modelExpressesMeasureDisplayFolders(model: TMDLModel): boolean {
  return model.tables.some((t) =>
    t.measures.some((m) => m.displayFolder && m.displayFolder.trim() !== ''),
  );
}
...
function hasLeadingTrailingSpace(name: string): boolean {
...
function hasFactDimPrefix(name: string): boolean {
...
interface NameSets {
  readonly measureNames: Set<string>;
  readonly columnNames: Set<string>;
  readonly columnRefs: Set<string>;
}

function buildNameSets(model: TMDLModel): NameSets {
  const measureNames = new Set<string>();
  const columnNames = new Set<string>();
  const columnRefs = new Set<string>();
  for (const t of model.tables) {
    for (const m of t.measures) measureNames.add(m.name);
    for (const c of t.columns) {
...
function columnTableFor(model: TMDLModel, columnName: string): string | undefined {
  for (const t of model.tables) {
    if (t.columns.some((c) => c.name === columnName)) return t.name;
  }
  return undefined;
}
...
function blankOutQualifiedRefs(expr: string): string {
  return expr.replace(/('[^']+'|[A-Za-z_][\w .-]*)\[[^\]]+\]/g, ' ');
}

function normalizeDax(expr: string): string {
  return (
    stripDaxComments(expr)
      .replace(/\s+/g, ' ')
