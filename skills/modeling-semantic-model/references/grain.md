...
If `pbi_model_plan_date_grain` returns `observedGrain: "day"`, the target/forecast/planning fact has enough repeated sub-month evidence to pass the write gate. In that case, do **not** add month-start truncation (`DATE(YEAR(...), MONTH(...), 1)`) or month-key `TREATAS`; use the date-level relationship plus shared dimensions and a plain additive target measure when the relationship plan allows it.
