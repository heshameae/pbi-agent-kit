...
| Position | Token | Examples |
|---|---|---|
| 1 (optional) | Aggregation | `MTD`, `QTD`, `YTD` |
| 2 (required) | Base Name | `Turnover`, `Net Sales`, `Selling Margin` |
| 3 (optional) | Period | `1YP`, `2YP`, `1MP` |
| 4 (optional, parenthesised) | Unit | `(%)`, `(EUR)`, `(Quantity)`, `(Value)` |
...
## Downstream Rename Impact Warning

Renaming measures, columns, or tables in an existing model has downstream consequences that must be considered before applying any naming fix:

- **Report bindings** — PBIR visual `queryRef` values reference field names by their current name; renaming breaks bound visuals in any report consuming the model
- **DAX expressions** — measures that reference other measures by name will break if the referenced name changes
- **Calculated columns** — column references inside DAX expressions use the `'Table'[Column]` syntax; renaming either breaks the reference
- **External tools** — Excel PivotTables, third-party BI tools, and embedded queries reference field names directly
- **Verified answers and AI instructions** — any field references in AI instructions text or verified answer configurations must be updated manually

**Rule:** Never silently rename objects in a production model. Present the rename plan to the user, list all downstream references found, and get explicit approval before applying. For models in development, rename freely but run `pbi_dax_reference_check` afterward to catch broken DAX references.
