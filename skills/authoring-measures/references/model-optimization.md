...
- **Fabric resource profile:** Use the `readHeavyForPBI` resource profile which enables V-ordering and optimized write settings automatically.

---

### DL002: Segment Size and Parallelism

Delta rowgroups map directly to VertiPaq segments — one segment per CPU core. More segments = better CPU saturation (see SE Parallelism Factor in `references/engine-internals.md`).
