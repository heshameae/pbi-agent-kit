...
**Source:** `plugins/reports/skills/pbi-report-design/references/page-titles.md` (data-goblin reports reference).
...
Use multiple `textRuns` within one paragraph to vary formatting across a single line (e.g. a regular word followed by a bold word):
...
Create a `textbox` visual.json file manually with position `x=24`, `y=24`, `width=500`, `height=48`. Set the paragraph content with the desired title text and font size (e.g. 24pt). See the `pbi-report` skill for the underlying PBIR visual.json structure.
